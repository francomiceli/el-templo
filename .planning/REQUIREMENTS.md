...
- [ ] **INFRA-03**: CI pipeline: type check, lint, build for el-templo-web (extend existing ci.yml)
- [ ] **INFRA-04**: Staging deploy pipeline: build → rsync to EC2 → Nginx config at staging.eltemplo.org
- [ ] **INFRA-05**: Production deploy pipeline: build → backup → rsync → Nginx at eltemplo.org → smoke test → auto-rollback
