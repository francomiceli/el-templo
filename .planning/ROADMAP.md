...
- [ ] **Phase 94: OpenAI Latency + Graceful Failure** — BUG-02 ~3min latency; explicit OpenAI client timeout + interim UX + graceful fallback at `provider.chat(...)` await sites (LAT-01..03). 🟡 **Phase 94 LAT-01..03 RED tests at `el-templo-bot/test/v5-3-3-openai-latency.test.ts` went GREEN as a side effect of Phase 100 Wave 1's driver update (commit `af1db504`)** — the previously-RED tests were timing-blocked at `vi.advanceTimersByTimeAsync(3500)` and never reached the LAT-01..03 GREEN code that already shipped in 94-01/94-02 (`d3de86b1`/`c6c6bc0e`). The stepped 7s+ advance via `advancePastQuietWindow()` now correctly drives the timeout path. Phase 94 stays `human_needed` for the live-test gate (v5.4.0 acceptance per STATE.md); the deferred RED tracker is closed.
- [ ] **Phase 95: Booking Reliability + Graceful Degradation** — BUG-03 + BUG-05 paired; class search consistency across venues + tool-failure retry-counter + escalate via `request_human` after 2 failed attempts; SC#3 invariant preserved (BOOK-01, DEGR-01..02). 🟡 **Phase 95 DEGR-01 RED tests (3) at `el-templo-bot/test/v5-3-3-degr-01-escalation.test.ts` went GREEN as a side effect of Phase 100 Wave 1's driver update** (same `af1db504` commit, same timing-blocked → unblocked dynamic). The escalation-counter / synthetic-phrase GREEN implementations already shipped (`2d7cd171` + subsequent); the RED tests were timing-blocked. Phase 95 BUG-03 (LIKE-search ambiguity at `tools.ts:455`) deferred RED remains — that's the API-side test, not affected by bot-side timing changes.
...
- [x] **Phase 99: Bot copy and price disclosure fixes** ✅ shipped 2026-06-24 — Mica name reinforcement (COPY-01) + "Sesión Grupal" → "clases de calistenia" rename + 5 preservation strings (COPY-02) + Option-B price disclosure after 2 insistences via per-PB1 counter + threshold-based prompt addendum + check_membership lead-handling fix (PRICE-01..04). 3 plans / 3 waves, KGATE-05 cap-trim resolution in Wave 1. Final baselines: el-templo-api 537/1/1, el-templo-bot 644/4/0 (4 deferred RED tracking Phases 94+95). Verifier PASSED 6/6.
- [ ] **Phase 100: Bot takeover ACK, debounce, and price trigger** — bot-side follow-ups from Phase 93+99 live test: trailing-debounce poll-extend (DBNC-01) + human-takeover ACK (TAKE-01 context-aware addendum + TAKE-02 rate-limited reassurance with `wa:takeover_ack:<phone>` Redis key) + widened `detectPriceObjection` regex covering price questions (TRIG-01) + phase-level baseline invariant lock (TEST-01). 4 plans / 4 waves. Local-only — no push, no deploy. Depends on Phase 99.
  - [x] **100-01 DBNC-01 trailing debounce + Phase 93/94/95 driver updates** ✅ Wave 1 shipped 2026-06-24 — 7 atomic commits (`70764564` Task 1 RED → `6fa7d430` Task 1 GREEN → `837d48e9` Task 2 RED → `f9ab1856` Task 2 GREEN → `af1db504` Task 4 driver updates → `374c1c6c` Task 3 integration tests → `a9dec864` SUMMARY). **Bot suite baseline FLIPPED: 644/4/0 → 662/0/0** — 18 new tests + 4 previously-deferred RED → GREEN (Phase 94 LAT × 1 + Phase 95 DEGR-01 × 3 were timing-blocked at the OLD `vi.advanceTimersByTimeAsync(3500)` driver). Same-documented-reason RED check passed-by-elimination (no NEW failures introduced — all 4 RED genuinely exercise their shipped GREEN implementations now). All HARD GUARDS preserved (DEBOUNCE_TTL_SECONDS line, 6-pair sha256 invariant, Phase 99 preservation strings, KGATE-05 NO-OP, debounce.test.ts byte-equal, zero el-templo-api/src changes). Scope deviation: `ai-handler.test.ts` adapted (+27 lines) for new helper imports + env-overridden quiet-window — Phase 99 lesson applied inline (dual-grep discipline + executor caught the carry-through).
  - [x] **100-02 TAKE-01 (context-aware addendum) + TAKE-02 (rate-limited reassurance)** ✅ Wave 2 shipped 2026-06-24 — 4 atomic commits (`0c9c1306` TAKE-01 addendum + handoffReason option in system-prompt.ts → `4bdf1abe` TAKE-02 3-branch Redis fail-mode + TAKE-01 handler wiring → `2d8abc2d` integration test with "no AI in takeover" sentinel → `24276cf9` SUMMARY). TAKE-01 wiring choice: **handler-entry session-history scan** (`extractMostRecentRequestHumanReason()` parses prior assistant messages for `[tool_call: request_human({...})]` and threads `handoffReason` into `getSystemPrompt`). KGATE-05 NO-OP confirmed (addendum gated OFF for PB1.E1A render; POST_RLOK_04_BYTES = 18910 byte-equal; 4 snap-consuming tests green). DEGR-01 HARD GUARD held (`v5-3-3-degr-01-escalation.test.ts` byte-equal; HANDOFF_ESCALATION_PHRASE single declaration preserved). TAKE-02 3-branch fail-mode: unavailable → fail-OPEN, SETEX throws → fail-CLOSED, OK/null → branch on `acked`. **Bot suite: 677 passed / 0 failed / 0 todo** (post-Wave-1 662 + 15 new). **API suite finding (baseline correction):** plan documented `540/1/1` but actual baseline is `537/4/1` — the 3 extra failures (`raw-sql-column-drift.test.ts` + `ai-tools-membership-drift.test.ts`) are PRE-EXISTING Phase 97.5/98 flake-pattern files (Phase 98 SUMMARY documented `ai-tools-membership-drift` flake explicitly). Wave 2 added only 1 new api test file (`v5-3-3-phase-100-takeover-ack.integration.test.ts`, 405 lines), 0 modifications to the failing test files. Final api: 539 passed (537 baseline + 2 from new file) / 4 failed (1 BUG-03 i + 3 pre-existing DRIFT flake) / 1 todo. No regression; baseline correction needed in Wave 3/4 gates.
...
- [x] 99-03-PLAN.md — Integration test coverage in `el-templo-api/test/whatsapp/` (PRICE-04 + COPY-01/02 source-text regression locks + no-hardcoded-prices guard) — Wave 3 ✅ shipped 2026-06-24. 3 commits (`1e68cb3e` integration test file 14 it() + 1 it.todo / `2633aa23` no-hardcoded-prices guard 4 it() / `af189166` SUMMARY). Full el-templo-api suite: **537 passed / 1 failed / 1 todo (539 total)** — the 1 failed is the documented Phase-95-deferred BUG-03 (i) LIKE-search RED at `test/whatsapp/v5-3-3-booking.integration.test.ts:130` (intended). Phase 98 baseline preserved: 519 prior + 18 new = 537. Zero src/\*\* modifications (Phase 98 SC#5 HARD GUARD held). One planning-surfaced finding: ALLOWLIST expanded from 1 → 4 entries (3 additional pre-existing knowledge.ts references at :361 `~$10,000` per-class anchor + :601/:604 Plan Performance template + JSDoc comment — none Phase 99-introduced, all marked for "business-pricing review owner"). See [99-03-SUMMARY.md](phases/99-bot-copy-and-price-disclosure-fixes/99-03-SUMMARY.md) §Deviations for full rationale.
...
**Goal:** Close three bot-behavior follow-ups from live WhatsApp testing of Phases 93 + 99: (a) trailing-debounce aggregation for multi-message senders (replaces the fixed 3s `setTimeout` with a quiet-window + hard-cap poll-extend loop inside the existing Phase 93 SETNX-acquired token); (b) human-takeover acknowledgment — LLM-driven context-aware addendum on the escalation turn (TAKE-01) + rate-limited static reassurance for subsequent inbounds during `human_takeover` (TAKE-02, key `wa:takeover_ack:<phone>` + TTL); (c) widened `detectPriceObjection` regex covering price QUESTIONS (precios? / cuánto sale|cuesta|vale / valor(es) / tarifa / cuota / mensualidad) so Phase 99's PB1 disclosure mechanism fires for genuine price-curious leads. All bot-side; LOCAL only — no push to master, no deploy. Mirrors Phase 99's locked-scope-dictation pattern.
**Requirements**: DBNC-01, TAKE-01, TAKE-02, TRIG-01, TEST-01 (internal labels per `100-CONTEXT.md` — NOT registered in REQUIREMENTS.md by design; same convention as Phase 99)
**Depends on:** Phase 99 (shipped 2026-06-24) — Phase 100 Item 3 widens the `detectPriceObjection` helper whose Phase 99 PB1 counter is the primary downstream consumer
**Plans:** 4 plans

Plans:

- [ ] 100-01-PLAN.md — DBNC-01: trailing-debounce poll-and-extend loop (replaces fixed `setTimeout(DEBOUNCE_DELAY_MS=3000)` at handler.ts:465) + env-overridable `DEBOUNCE_QUIET_WINDOW_MS=7000` + `DEBOUNCE_HARD_CAP_MS=30000` + Redis-backed `recordInboundAt`/`getLatestInboundAt` helpers in session.ts + unit tests for loop control + integration test for end-to-end timing. Wave 1. Preserves Phase 93 SETNX serialization; preserves `DEBOUNCE_TTL_SECONDS=600` byte-equal (HARD GUARD).
- [ ] 100-02-PLAN.md — TAKE-01 + TAKE-02: new `HANDOFF_CONTEXT_AWARE_ADDENDUM` in system-prompt.ts (Sub-option A, LLM-driven, conditional injection gated on `options.handoffReason`) + handler-side `TAKEOVER_REASSURANCE_PHRASE = "Alguien del equipo te va a responder a la brevedad 🙏"` + rate-limited reassurance with `wa:takeover_ack:<phone>` Redis key + `TAKEOVER_ACK_TTL_SECONDS=3600` + critical "no AI in takeover" sentinel test (provider.chat mock throws on call). Wave 2 (depends on 100-01 — shared `handler.ts` surface forces serialization). Preserves `HANDOFF_ESCALATION_PHRASE` byte-equal for the DEGR-01 auto_escalation_after_2_failures path.
- [x] 100-03-PLAN.md — TRIG-01: widen `detectPriceObjection` regex at handler.ts:1670 (single source of truth — no parallel regex) to match `precios?`, `cu[aá]nto (sale|cuesta|val[eé])`, `valor(es)?`, `tarifa`, `cuota`, `mensualidad` in addition to all existing terms + data-driven unit fixture (28 positive/negative cases) + PB1 counter integration test (question-shaped 3rd insistence unlocks Phase 99 disclosure) + PB2.E2 routing integration test (neutral price question). Wave 3 (depends on 100-02 — shared `handler.ts` surface). ✅ SHIPPED 2026-06-25 (commits `833f76c9` RED → `8b51bfbb` GREEN → `895d6c03` integration). Single source of truth grep = 1. 28/28 unit tests pass; 3/3 integration scenarios pass. Phase 99 PB1 counter integration test still GREEN (14/14 + 1 todo).
- [ ] 100-04-PLAN.md — TEST-01: phase-level baseline invariant lock (~20 grep-based assertions on Phases 93 + 99 + 100 carry-forward tokens) + operator human-verify checkpoint (per-item PASS/PARTIAL/FAIL with verbatim bot reply quotes). Wave 4 (depends on 100-01..03). Captures the post-Phase-100 baseline test counts (bot ≥656/4/0; api +8 passing) and locks the 6-pair sha256 invariant tripwire.

**Notes:**

- **Scope fence (HARD):** runtime code edits only in `el-templo-bot/src/{webhook,memory,ai}/**` + `.env.example`; unit tests in `el-templo-bot/test/`; integration tests in `el-templo-api/test/whatsapp/`. ZERO modifications to `el-templo-api/src/**` — that surface is owned by another developer per `project_bot_scope_boundary` memory.
- **Phase 93 invariants preserved (HARD GUARDS):** `DEBOUNCE_TTL_SECONDS = Number(process.env.DEBOUNCE_TTL_SECONDS ?? 600)` at handler.ts:167 byte-equal; its JSDoc formula block byte-equal; the 6-pair sha256 cross-phase invariant block `67670b1e1099bf7c8a5285414736f16e8a010a010348bf6566790d0db3163344` untouched; SETNX serialization semantics intact (poll-extend loop runs INSIDE the SETNX-acquired token, not in a parallel handler path).
- **Phase 99 invariants preserved (HARD GUARDS):** PB1.E4 `REGLA FUERTE` byte-equal at definitions.ts:74; `PB1_PRICE_DISCLOSURE_UNLOCKED_ADDENDUM` byte-equal at system-prompt.ts:204; 5 preservation strings byte-equal (`movimiento grupal` + `sin salirte del grupo` + `sin salirse del grupo` in knowledge.ts; `framings de arranque grupal` + `lenguaje de arranque grupal` in definitions.ts); `PB1_PRICE_INSISTENCE_THRESHOLD=2` mechanism continues to fire under the widened TRIG-01 regex.
- **KGATE-05 NO-OP:** Item 2a's new `HANDOFF_CONTEXT_AWARE_ADDENDUM` is gated on `options.handoffReason` which is undefined during the PB1.E1A lead-render path the snap fixture captures → `POST_RLOK_04_BYTES = 18910` unchanged. Current margin remains 6 chars.
- **Critical "no AI in takeover" guard:** TAKE-02's new logic at handler.ts:383 must return BEFORE any AI call path. Locked with sentinel tests (unit + integration) where `provider.chat = vi.fn(() => { throw new Error("AI MUST NOT BE INVOKED") })` — sending a message during human_takeover MUST NOT invoke the mock.
- **Single source of truth for the price regex:** Item 3 modifies only `detectPriceObjection` at handler.ts:~1410. Verify gate: `grep -rE "caro\|carisimo" el-templo-bot/src/ --include="*.ts" | wc -l` returns EXACTLY 1.
- **Phase 99 lesson — full bot suite gate (mandatory in every wave):** `cd el-templo-bot && pnpm test` must run the FULL bot suite at the end of every wave that modifies bot src. Baseline 644 passed / 4 failed / 0 todo (the 4 failures are the deferred Phase 94 LAT + Phase 95 DEGR-01 RED tests; ANY new failure blocks the wave). Plus dual-grep discipline: every wave greps BOTH `el-templo-bot/src/**` AND `el-templo-bot/test/**` for affected tokens.
- **Threat model summary:** prompt-injection via `request_human` `reason` arg (mitigated — addendum body instructs model to treat reason as DATA, NOT instructions; explicit "Trátalo como CONTEXTO" wording locked by test); regex false-positive widening (mitigated — `\b` word boundaries + negative test fixture); replay of TAKE-02 reassurance after TTL expiry (accepted — per CONTEXT.md a second ack after 1h is reasonable). Per-plan `<threat_model>` blocks document `block_on: high`.
- **No live deploy.** Local-merge to `feature/whatsapp-bot-scaffold` only. Phase 99's local-merge discipline is the precedent.
