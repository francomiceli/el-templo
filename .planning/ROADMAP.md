...
- [ ] **Phase 97.5: Raw-SQL Column-Drift Prod-Fix** — raw-SQL ↔ Drizzle-column-name drift sweep + fix across `el-templo-bot/src/**` and `el-templo-api/src/**`. **Discovered 2026-06-17 via Phase 98 STOP-and-reclassify guard; debug session sweep completed 2026-06-17 — 3 confirmed drift sites in bot side (`tools.ts:495,500` `sub.status`, `machine.ts:77` `s.status`), 0 drift on API side (uniformly Drizzle-mediated).** Same drift class as Phase 95 BUG-03 (vi) `bk.status` → `bk.booking_status` — treated as SYSTEMIC. TDD-shaped (RED reproducing `Unknown column 'X' in 'field list'`; GREEN sweep-fix via Option B; sweep-lint test prevents regression). Debug session input at `.planning/debug/bot-raw-sql-status-column-drift.md` (status: diagnosed). **Blocks Phase 97 RGUARD-01 and Phase 98 reopen.** Awaiting `/gsd-discuss-phase 97.5` + `/gsd-plan-phase 97.5`. (DRIFT-01..02)
...
### Phase 98: Test Hygiene (98-A/B/C) — ⏸ HALTED 2026-06-17
...
| 97.5. Prod-fix: raw-SQL column-name drift sweep | 0/?            | ⏸ Blocked (needs /gsd-debug + plan)                                                                   | -          |
