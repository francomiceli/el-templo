...
- [ ] 135-01-PLAN.md — Block A: dry-run plan printer + transactional --apply (bulk accept pending milestone proposals) + idempotency test
- [ ] 135-02-PLAN.md — Block A→prod: catalog-parity check (D-07) + 0146 DATA migration populating milestone_exercise_id + rollback doc
- [ ] 135-03-PLAN.md — Block B backend: variants[] embedded in GET /tree via separate batched query (backbone predicate untouched) + no-regression test
