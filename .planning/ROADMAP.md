...
- [ ] 100-03-PLAN.md — TRIG-01: widen `detectPriceObjection` regex at handler.ts:~1410 (single source of truth — no parallel regex) to match `precios?`, `cu[aá]nto (sale|cuesta|val[eé])`, `valor(es)?`, `tarifa`, `cuota`, `mensualidad` in addition to all existing terms + data-driven unit fixture (~18 positive/negative cases) + PB1 counter integration test (question-shaped 3rd insistence unlocks Phase 99 disclosure) + PB2.E2 routing integration test (neutral price question). Wave 3 (depends on 100-02 — shared `handler.ts` surface).
