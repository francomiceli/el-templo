...
status: executing
stopped_at: Completed 172-19-PLAN.md
...
Next: `/gsd:execute-phase 172` sigue por el plan **172-20** (el gate consolidado de cobertura de la batería). **El 172-19 CERRÓ la batería ISO-03: 14 + 13 + 11 = las 38 rutas finance `tenant-scoped` del manifiesto tienen caso** (verificado por script contra `test/tenant-manifest.ts`; la tabla ruta→archivo está en el `172-19-SUMMARY.md` y es la que consume el 172-20). Este plan cubrió las 11 que faltaban —las 7 de `/coach-load/*` con el token de un **COACH** (D-10, el borde menos privilegiado real, y el token va explícito en cada call site) y las 4 de movimientos y egresos con `adminToken` (viven en `finance/routes.ts`, cuyo hook excluye a coach: admin ES el mínimo disponible)—: **31 tests / 12 describes**, cero 403 esperados, y como evidencia el **SALDO de las cajas** leído de la base (este grupo no tiene socio: `member_id NULL`, así que no hay listado que mirar). **🔴 LO IMPORTANTE NO ES EL VERDE: bajando al rol coach aparecieron DOS deudas de `subscriptions`, las dos fuera de alcance por D-07 y las dos ancladas con aserciones ejecutables que se ponen en ROJO cuando la fase 173 las arregle.** **(1) FUGA REAL:** `getMemberSubscription(userId)` es la única llamada de `GET /coach-load/autocompletar/:userId` **sin `ctx`**, así que la ruta devuelve `planName`, `amount`, `currency` y `currentEndDate` de un socio de OTRO gimnasio **al coach**, iterando ids (reproducido, no inferido; lo que sí aísla es `memberBranchId`, que lleva su `tenantWhere` desde el 172-11). **(2) BLOQUEO DE ADOPCIÓN:** `assignPlan` inserta `subscriptions` **sin `tenantValues`** → la sub de un gimnasio nuevo nace con el `DEFAULT 1`, el charge la rechaza como concepto enlazado inexistente y rollea todo: **`POST /coach-load/alta` no es usable por un gimnasio que no sea El Templo** (fail-closed, no es fuga — el test verifica que no queda rastro en ninguno de los dos). Por eso el control positivo del alta corre en El Templo. **(3) REQUISITO DE ADOPCIÓN que nadie había escrito:** un gimnasio nuevo necesita **su propia sede virtual 'Templo Online'**, porque el fallback de `resolveUserBranchId` la busca por nombre dentro del gimnasio y sin ella las rutas de carga contestan error del servidor en vez del payload vacío del contrato → va a la receta del **172-23**. Las tres están en `deferred-items.md` con el nombre del `it` que las ancla. **Y el hallazgo técnico del plan corrige al 172-18:** en este universo el `tenantWhere` de la tabla **NO** es la única barrera. Hicieron falta **CINCO mutaciones simultáneas** para que un movimiento cruce gimnasios (con tres, solo caía el caso de `autocompletar`): el camino está cortado por `resolveCajaCountry` (que filtra la caja **y** la sede en el `ON` del leftJoin), `loadCaja`, y el chequeo de sede de `create` — y para la combinación inversa hay una quinta en `getBalance`. Con las cinco rotas el movimiento cross-tenant se ejecuta de verdad (201, asiento contra la caja de El Templo) y los 3 tests que tienen que morder, muerden. La mutación quedó revertida y verificada por `md5sum` de los 4 archivos. **Corridas:** el archivo solo da 31/31 (~152 s) y **los TRES archivos iso-03 en un solo worker (`--no-file-parallelism`) dan 98/98** — esa es la prueba de no-contaminación que vale; `test/tenancy` entero en paralelo sigue sin entrar en esta máquina (deferred del 172-18, importa para el 172-21). **El 172-18 cerró las 13 rutas del corazón transaccional** (transacciones, bandeja de pendientes, arqueo por caja y los 3 exports): **33 tests / 14 describes**, aislamiento + control positivo por ruta, **cero 403 esperados**, y toda escritura verificada releyendo la FOTO de la transacción ajena de la base (gimnasio, estado, anulación, caja imputada e importe — cinco columnas en un solo `toEqual`). Aporta la técnica para probar lo que **no devuelve filas**: importes irrepetibles y de otro orden de magnitud por gimnasio, así un `/transactions/summary` contaminado se va a los millones y el rojo se lee solo; los tres `.xlsx` se parsean y se afirman **por contenido**; y los **contadores** (`total` de la bandeja y del arqueo) se afirman aparte de las filas, porque son otra query. Encontró dos vectores que solo se ven leyendo el código: la **caja ajena** solo es alcanzable por `validate({cashRegisterId})` (el body del alta NO acepta caja: la resuelve el servidor desde medio de pago + sede) y el **socio ajeno** por `correct({correctedFields.memberId})`, que además prueba el rollback del anular+recrear. **Y un hallazgo que el 172-19 tiene que usar: la mutación de cierre puso 2 rojos (`validate` y `listPendingMiscForMember`) pero NINGUNO al sacarle el `tenantWhere` a `buildListConditions`** — el aislamiento de `GET /transactions` y su export está sostenido por DOS filtros independientes (los INNER JOIN de `users`/`branches` llevan el suyo), o sea que el de la tabla es defensa en profundidad **ahí**; en el universo del 19 (egresos y traspasos con `member_id NULL`, listados con LEFT JOIN) esa segunda barrera **no existe** y el `tenantWhere` de `financial_transactions` es la única — la mutación del 19 tiene que apuntarle a ella. **Ojo con las corridas: `test/tenancy` entero EN PARALELO no entra en esta máquina** — 7 archivos rojos por provisioning de las bases por worker (bases sin migrar 0196/167, `beforeAll` timeout, `dbPool` undefined), con el archivo del 172-17 —conocido verde— entre ellos; los dos archivos iso-03 en **un solo worker** (`--no-file-parallelism`, base compartida) dan **67/67**. Anotado en `deferred-items.md`: **el 172-21 corre la suite completa con el sentinel en throw y este ruido se confunde con un rojo del switch**. Bandera para después de la fase (no de esta): `canAccessBranch` (`src/modules/shared/branch-access.ts`) resuelve la sede **sin filtro de gimnasio** y para un admin decide por país, así que `requireBranchAccess` deja pasar una sede ajena del mismo país; en finance lo frena el guard del handler, pero **toda ruta que use ese preHandler como único guard de sede es un agujero** (fuera de alcance por D-07 — material de la 173). **El 172-17 arrancó la batería ISO-03 y quedó verde**: 34 tests / 15 describes que cubren las **14 rutas de cajas y centros de costo** (5 GET + 9 escrituras), cada caso de aislamiento con su control positivo, contrato D-09 respetado (404/vacío, **cero 403 esperados**) y toda escritura verificada releyendo la fila ajena **de la base**, no del status HTTP. Se cerró con **mutation testing**: romper a mano 2 `tenantWhere` de `cash-register-service.ts` pone **7 tests en rojo**, exactamente en las rutas que sirven los métodos mutados — el verde no es un placebo (la mutación quedó revertida y verificada). Deja además el fixture que consumen el 18 y el 19 (`sembrarFinanzasGimnasioDos` / `sembrarFinanzasTemplo` / `tenantDeLaFila` / `campoDeLaFila` sobre las 6 tablas strict) y un hallazgo que hay que copiar: **`limpiarFinanzasDeLaBateria` corre ANTES de `seedSecondTenant`**, porque `cost_centers` no está en `TABLES_TO_CLEAN` ni en `limpiarSegundoGimnasio` y su FK a `tenants` hace reventar el `DELETE FROM tenants` con `ER_ROW_IS_REFERENCED_2`. **La cadena serializada 13→14→15→16 quedó CERRADA con el 172-16**: el barrido global sobre TODO `test/` (245 archivos) sale en **cero** —no queda una sola query literal sobre las 6 tablas strict sin tenant ni exención— y la corrida en caliente dio **649/652 con CERO throws del sentinel**. Los 3 rojos que quedan son los tres `it` de `test/tenancy/con-06-lint.test.ts` y **le pertenecen al 172-21**: dos reproducen con la sonda APAGADA (son la deuda de allowlist de la fase, 51 entradas stale + la lente estática que bajó de 87 a 81 tablas porque `src/` está migrado) y el tercero es el gate D-15 que afirma que `TENANT_STRICT_MODULES` sigue vacía. El 172-16 además arregló **2 rojos de la fase que nadie había visto**: el gate `iso-02 Test 13` estaba rojo desde el 172-13 (un comentario de `test/setup.ts` nombraba el módulo del fixture y el gate busca por substring) y el barrido de integridad de `verifyTenantBackfill` hacía throw sobre 3 tablas strict. El **172-15 se cerró por safe-resume** (la sesión anterior dejó los 3 commits hechos y la sonda viva sin SUMMARY: se re-verificó en caliente —54 archivos / 639 tests verdes, 0 throws—, se revirtió la sonda y se escribió el SUMMARY; incluye el fix `4c252510` de la regresión `assertTenant` sin `attachCountryScope` en la ruta de add-ons de programas). **`finance` quedó CERRADO con el 172-12** (cero entradas vivas de allowlist en los 6 archivos del módulo, `strictWithAllowlist` 0) y con el **172-13** arrancó la cadena SERIALIZADA de endurecimiento de tests (13→14→15→16), que comparte la sonda temporal sobre `src/db/tenant-tables.ts`: **pre-check obligatorio de `git status --porcelain el-templo-api/src/db/tenant-tables.ts` (tiene que salir vacío) antes de encenderla**. En paralelo sigue pendiente el plan **169-09**, el último de la fase 169 (gate consolidado + rollout), y siguen pendientes
...
**172-13 cerrado — la infraestructura de test sobrevive al throw, y la deuda de allowlist NO se movió (sigue en 51) a propósito.** `cleanAllTestData` —el `beforeEach` de decenas de archivos, un `DELETE FROM` sin filtro sobre ~70 tablas, tres de ellas strict, **por el pool que el sentinel envuelve**— era el bloqueante #1 del switch: sin este plan, el 172-21 no habría visto "40 archivos rojos" sino **la suite entera caída en el primer hook**. Su statement lleva ahora la exención **escrita ADENTRO DEL SQL** (`DELETE /* tenant-safe: … */ FROM …`), que es el único canal que el sentinel lee — **la allowlist del lint NO sirve para esto: son dos canales distintos (D-17 de la 170) y esa allowlist ni siquiera mira `test/`**. Probado ejecutando el clasificador real: `analyzeSql` devuelve `exempt` con motivo sobre el statement anotado y `violation` sobre el mismo sin anotar. **La regla para decidir, que vale para los planes 14/15/16:** si el acceso global es DELIBERADO → exención con motivo; si se puede acotar sin cambiar lo que el test prueba → filtro (por eso los 3 `sql` crudos del `beforeEach` de `validate-caja` pasaron a `WHERE tenant_id = ${TENANT_TEMPLO}` en vez de llevar exención). **Los statements sobre `users` NO se anotaron a propósito**: `users` no es strict hoy y una exención heredada sin dueño es cómo se apaga un tripwire solo. Además: `ensureEfectivoCaja` tenía un throw esperándolo (su sonda de idempotencia sobre `cash_registers` no filtraba) y los 4 seeds de `cash_registers` de `test/setup.ts` estampan `tenant_id` explícito con sus `NOT EXISTS` filtrados — **ojo, `setup.ts` NO pasa por `app.dbPool`** (crea sus propias conexiones mysql2), así que el sentinel no lo ve: se migró por el riesgo real, no por el vigilante. **80 queries directas migradas** en los 6 archivos de cajas/cuentas/centros de costo (cash-register-service 35 · cash-balances 13 · cost-centers 12 · validate-caja 11 · cost-centers-abm 6 · bank-accounts 3), **cero expectativas tocadas**, y **93/93 tests verdes contra MySQL real CON `finance` puesto a mano en `TENANT_STRICT_MODULES`** (30+37+26 en tres corridas). **La sonda se revirtió** y `git status --porcelain … tenant-tables.ts` sale vacío. Dos cosas más para copiar: `tenantValues` en un insert multi-fila envuelve **cada fila** del array, no el array; y la cobertura se auditó con un `awk` que exige `tenantWhere`/`tenantValues`/`tenant_id` en la ventana de cada acceso a tabla strict y **lista la línea** de los que faltan (un archivo de 858 líneas con 35 accesos no se revisa a ojo). Commits: `e3092ea6`, `4a3244fe`. **ADO-01 sigue Pending.**
