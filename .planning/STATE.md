...
**Current focus:** v6.0 cerrado — aterrizar 179/180 (fuera del milestone) y planificar el próximo milestone (Módulo Gimnasio / onboarding tenant 2)
...
Ítems reconocidos y diferidos al cierre del milestone v6.0 (2026-08-26), vía `gsd-sdk query audit-open` (83 en total):

| Categoría         | Cantidad | Nota                                                                                 |
| ----------------- | -------- | ------------------------------------------------------------------------------------ |
| uat_gaps          | 31       | UATs históricos de fases pre-v6.0 (los pendientes de UAT ya trackeados por proyecto) |
| verification_gaps | 49       | VERIFICATION.md faltantes/parciales de fases viejas ya shippeadas                    |
| quick_tasks       | 2        | `001-remove-timers-add-saberes-info`, `2-create-coach-user-guide-for-el-templo-ad`   |
| todos             | 1        | `v51-milestone-data-rollout.md` (poblar `milestone_exercise_id`, high)               |

## Performance Metrics

**Velocity:**

- Total plans completed: 120 (v4.1)
...
| Phase 172 P18 | 60min | 2 tasks | 1 files |
| Phase 172 P19 | 115min | 2 tasks | 1 files |
| Phase 172 P20 | 30min | 2 tasks | 3 files |
| Phase 172 P21 | 40min | 3 tasks | 4 files |
| Phase 172 P22 | 120min | 3 tasks | 2 files |
| Phase 172 P23 | 35min | 2 tasks | 1 files |
| Phase 173 P01 | 75 | 3 tasks | 4 files |
| Phase 173 P03 | 95min | 3 tasks | 2 files |
| Phase 173 P02 | 95min | - tasks | - files |
| Phase 173 P02 | 95min | 3 tasks | 1 files |
| Phase 173 P23 | ~6h | 2 tasks | 64 files |
| Phase 177 P01 | 15min | 2 tasks | 4 files |
| Phase 177 P02 | 20min | 2 tasks | 2 files |
| Phase 177 P03 | ~10min | 2 tasks | 1 files |
| Phase 177 P04 | 25min | 2 tasks | 1 files |
| Phase 177 P05 | 55min | 3 tasks | 1 files |

## Accumulated Context

### Roadmap Evolution

- Phase 180 added (Campañas etapa 2 de la fase 119; ejecutar POST-tren v6.0 en master — toca member app, bump minor + builds de tiendas; standalone respecto de la 179): magic links por sede (token one-use 30d que loguea al destinatario, revisar D-21; CTA primario a la sede del freemium + tabla de sedes + WhatsApp terciario; motivación: JWT 7d → freemium deslogueados → el login mata el funnel click→reservó) + selector de audiencias (bajas, prueba-no-convertida, alerta/ausente de la 136, referidos) + ReservasPage informativa para nuevos (dirección+"Cómo llegar", card "¿Primera vez?", confirmación post-reserva enriquecida con calendario anti no-show, descripciones por actividad con columna `description` + admin + bottom sheet; TODAS las clases aptas para todos los niveles → mensaje global sin chips). Scope aprobado por Nacho 2026-08-20; detalle en memoria project_phase_119_freemium_trial_campaign. (CAMPANAS-E2)
- Phase 179 added (standalone api/app/admin, NO depende del tren v6.0; **NO tocar `referrals` ni `computeReferralDiscountPercent`**): Referidos de partners/marcas — co-marketing con comercios locales. **Decisiones cerradas con el usuario 2026-08-22:** (1) entidad `referral_partners` separada de `users` (name, `code` varchar16 UNIQUE con vanity codes, `benefit_type`+`benefit_value`, `commission_type`+`commission_value`, `branch_id` opcional, active, contacto); (2) tabla espejo `partner_referrals` (partner_id, referred_id UNIQUE, status pending|qualified|revoked) con exclusividad cruzada contra `referrals` (un socio = un origen, 409); (3) beneficio del referido **configurable por partner**: descuento % primera(s) cuota(s) O pase/semana gratis (integra trials v5.8 / aura); (4) comisión **monto fijo one-time** por membresía confirmada (mismo trigger `qualifyFirstPayment`, primer pago > 0) → `partner_commissions` (subscription_id UNIQUE idempotente, status pending|settled|void) — el sistema NO mueve plata, registra deuda y liquidación manual en admin; (5) flujo QR: tarjeta física → URL registro `?ref=CODE` (la app ya lee `route.query.ref` en RegisterPage.vue), `resolveReferralCode` pasa a resolver en dos espacios (users + partners), generador de QR calcado de `tools/generate-branch-qrs.ts`; (6) admin v1: CRUD partners + conversiones por partner + pantalla de liquidación. Mapa completo del sistema actual y sus 8 acoples a `users` en el BRIEF del phase dir. Migración: numerar next-after-master y verificar colisión con trenes pendientes (0208 tomado por analytics). (PARTNER-REF)
- Phase 178 added (standalone TV/sessions, NO depende del tren v6.0; base = worktree `et-bloque-alt` / `feat/tv-bloque-alternativo` desde `origin/master` @ `32db2f16`): Bloque alternativo en técnica/combos + dos deuteros en pantalla del TV + baja de la rotación automática. **Decisiones cerradas con Nacho 2026-08-19** (plan seed: `~/.claude/plans/atomic-painting-beaver.md`): (1) rol nuevo `COMBOS_II_ALT`/`TECNICA_II_ALT` **generado desde el arranque** como 5º bloque (estructura fija técnica/combos pasa de 4 a 5 bloques; INITIUM→II_I→II_II→II_ALT→STRETCHING), reusa mismo route/formato forzado que II_II con hash que incluye el rol → ejercicios distintos; reusa el editor entero (una card por bloque). Etiqueta "COMBOS II ALT"/"TÉCNICA II ALT". (2) Alt en TV = **toggle de vista** sobre el 2º bloque (mostrar principal/alternativo) que **comparte cronómetro** — mismo mecanismo `displayRole` efímero que la rotación; nuevo `show_alternative` boolean en `tv_class_state` + botón en `TvControlPage`. (3) Deuteros regular = **2×2 = 4 listas** (2 deuteros × 2 niveles del par); `buildColumns` deja de colapsar vía `visualGroupOf`, `.stage` pasa a grid + rediseño de escala. (4) **Se saca la rotación automática** (acoplada con el 2×2, shippean juntas) + **drop de columnas** `deuteros_auto_rotate`/`deuteros_pinned_at` en la misma migración. Toca api (generador combos/tecnica, validadores block/session incl. conteo 4→5, tv/service+roster+types+schemas, migración 0206±), admin (3 diccionarios de labels + PDF `session-data-transformer` + `TvScreen/TvControl` + `render.ts` + `edit-service.blockMap`), app (labels). Migración: numerar next-after-master (0205 tope) y **verificar colisión** con trenes pendientes. Gobernanza: staging muy atrás del tren TV/159/160 → back-merge master→staging pendiente; push suelto `et-kinesis-label` a coordinar. (TV-BLOQUE-ALT)
- Phase 177 added (standalone, NO depende del tren v6.0 166–175.1; el CLI puso "Depends on: 176" pero se corrigió a standalone en el ROADMAP): Paquete de clases — alta de corto plazo a medida. Vende paquetes 1–3 semanas × 1–6 clases/semana desde el alta con precio por fórmula, SIN motor de precios en runtime. **Modelo B (cerrado con Franco 2026-08-14):** matriz de 18 planes reales generada por migración (categoría nueva `paquete`, `bookingMode='flexible'`, `durationDays=semanas×7`, `classesPerWeek=frecuencia`), oculta del picker normal; la UI del alta (CobrosPage PoS + AssignPlanDialog admin) muestra 1 producto con 2 selectores que resuelven al `planId`. El detalle mágico: `durationDays=semanas×7` hace que la derivación existente `ceil(durationDays/7)×classesPerWeek` dé exactamente `semanas×frecuencia`=total de clases → budget/tope-semanal/cobertura/renovación nativos, cero cambios en `assignPlan`. Fórmula: `tarifa_por_clase(frecuencia)×total_clases×recargo_corto_plazo(semanas)`, anclada a mensuales reales con piso (monótona + nunca canibaliza el mensual). Tarifas/recargos = knob de negocio (Franco). Preguntas abiertas para discuss/plan: España/EUR, descuentos Aura/referidos, priceZero/tarjeta, UX de renovación, verificar enforce del tope semanal en `flexible`, nombres visibles. Brief: `BRIEF-PAQUETE-CLASES.md`. Numeración de migración a coordinar con el tren (0198+). (PAQUETE-CLASES)
...
- Phase 174 edited: acotada a adopción de código (retro post-173: fases de 31 planes son inmanejables); batería+switch se mueve a 174.1
- Phase 175 edited: acotada a adopción de código; batería+switch+cierre de gates se mueve a 175.1; depende de 174.1
...
- [Phase ?]: 172-16: TERCER punto ciego del inventario por grep — el statement cuyo nombre de tabla NO esta en el fuente (sql.raw(tabla) sobre una union cerrada, o SQL armado por un helper de src/). La auditoria se parte en dos barridos: literal (veredicto) y dinamico (triage manual)
...
- [Phase ?]: 172-18: un AGREGADO no devuelve ids sino un numero, asi que solo se puede afirmar con importes IRREPETIBLES y de otro orden de magnitud por gimnasio — el total contaminado se va a los millones y el rojo se lee solo (y hace falta una precondicion que verifique que los importes siguen siendo distintos)
- [Phase ?]: 172-18: el COUNT de un listado paginado se afirma APARTE de las filas — son dos queries, y un filtro que viva en una sola deja al staff viendo 3 filas y un total de 6
- [Phase ?]: 172-18: la evidencia de tampering sobre plata es una FOTO de varias columnas comparada con un solo toEqual (gimnasio, estado, anulacion, caja imputada e importe) — un rechazo que ya escribio la mitad se ve igual que uno limpio si se mira una sola columna
- [Phase ?]: 172-18: una mutacion que NO pone nada en rojo tambien informa — sacarle el tenantWhere a buildListConditions no rompio un solo test porque los INNER JOIN de users/branches ya lo llevan: el filtro de la tabla es defensa en profundidad AHI, y deja de serlo en cuanto un listado usa LEFT JOIN o no tiene socio (egresos/traspasos del 172-19)
- [Phase ?]: 172-18: los vectores cross-tenant reales no siempre estan en la ruta obvia — POST /transactions no acepta cashRegisterId (lo resuelve el servidor), asi que la caja ajena solo se alcanza por validate({cashRegisterId}), y el socio ajeno por correct({correctedFields.memberId})
- [Phase ?]: 172-18: correr test/tenancy entero en paralelo revienta el provisioning de las bases por worker en esta maquina (7 archivos rojos, ninguno de tenancy) — la prueba de no-contaminacion que SI vale es correr los archivos en UN solo worker (--no-file-parallelism): comparten la misma base y corren en orden, que es exactamente el escenario del Pitfall 10
- [Phase ?]: 172-18: canAccessBranch (shared/branch-access.ts) resuelve la sede SIN filtro de gimnasio y para un admin decide por pais — requireBranchAccess deja pasar una sede ajena del mismo pais, asi que toda ruta que lo use como UNICO guard de sede es un agujero (en finance lo frena el guard del handler)
- [Phase ?]: 172-19: bajar de rol es lo que encuentra los agujeros — las 27 rutas anteriores de la bateria corrieron con admin y no vieron nada; con el token de un COACH aparecieron una fuga real (getMemberSubscription sin ctx filtra el plan de un socio ajeno) y un bloqueo de adopcion (assignPlan inserta subscriptions sin tenantValues y el alta de coach-load no se puede completar en un gimnasio nuevo)
- [Phase ?]: 172-19: cuando la bateria encuentra deuda de OTRO modulo que la fase no puede arreglar (D-07), se ancla con una asercion EJECUTABLE que afirma el contrato correcto y esta marcada como fallo esperado — el dia que la arreglen el test se pone en rojo y obliga a desmarcarlo; un comentario no hace eso
- [Phase ?]: 172-19: cuando el control positivo no se puede escribir en el gimnasio 2 por deuda ajena, se escribe en el gimnasio 1 — sigue matando el falso verde que el control existe para matar ('la ruta esta rota para todos')
- [Phase ?]: 172-19: el MOTIVO del rechazo se afirma junto con el status — una ruta con dos formas de contestar 'no encontrado' puede pasar el caso de aislamiento por el motivo equivocado (fue exactamente lo que pasaba en /coach-load/alta)
- [Phase ?]: 172-19: la evidencia de un asiento de doble entrada es el SALDO de las DOS cajas antes y despues, sumado de la base SIN filtro de gimnasio: filtrarlo esconderia justo la fila colada de otro gimnasio, que es lo unico que hay que cazar
- [Phase ?]: 172-19: apuntarle a UN tenantWhere y ver que no pasa nada NO significa que el test no muerda — el camino de un movimiento cross-tenant esta cortado por CUATRO filtros en serie (resolveCajaCountry por caja y por sede, loadCaja, y el chequeo de sede de create) y solo cae con los cuatro rotos a la vez
- [Phase ?]: 172-19: un gimnasio nuevo necesita su propia sede virtual 'Templo Online' — el fallback de resolveUserBranchId la busca por nombre dentro del gimnasio, y sin ella las rutas de coach-load contestan error del servidor en vez del payload vacio del contrato (requisito de adopcion, va a la receta del 172-23)
- [Phase ?]: 172-20: un gate que lee TESTS por texto tiene que borrar los comentarios primero — los 3 archivos de la bateria iso-03 listan sus 38 rutas en sus propios docblocks, asi que sin eso el gate da VERDE midiendo prosa aunque la bateria este vacia (es el 172-16 al reves: alla el substring ponia rojo de mas, aca pondria verde de mas y no lo descubre nadie)
- [Phase ?]: 172-20: el criterio de un gate de cobertura se DERIVA del manifiesto (categoria + prefijo), nunca se escribe a mano — con la lista escrita a mano hay dos registros que se desincronizan y el gate termina defendiendo el suyo en vez de la bateria; el nombre del describe (con la clave literal adentro) ES el registro y no puede mentir sin que el rojo lo diga
- [Phase ?]: 172-20: toda lista de excepciones necesita su propio gate (que la ruta siga existiendo, que siga FUERA del criterio automatico, que siga con motivo utilizable) — si no, agregar una linea ahi es la salida barata para apagar el rojo, y el gate entero deja de significar algo
- [Phase ?]: 172-20: un describe.skip no puede contar como cobertura, o existe una salida de UNA LINEA para el rojo: apagar el test en vez de escribirlo
- [Phase ?]: 172-20: el motor de un gate se prueba con fixtures sinteticos aunque el plan no lo pida — los tests que comparan dos listas derivadas siguen VERDES si la derivacion se rompe y las dos quedan vacias; el baseline y el 'cada fuente aporta > 0' son lo que convierte esa vacuidad en rojo
- [Phase ?]: 172-20: para auditar archivos de test se lee el FUENTE con readFileSync, no se importan — importarlos los EJECUTA (los 3 de la bateria tardan ~250 s contra MySQL) y un gate que cuesta eso deja de correr, que es lo mismo que no existir
- [Phase ?]: 172-21: EL SENTINEL DE RUNTIME EVALUA POR QUERY, NO POR TABLA — un solo tenant_id en la zona de predicado (el de un innerJoin a otra tabla) marca ok TODA la query aunque la tabla strict quede sin filtrar; sacarle el tenantWhere a getSummary() dio 5/5 verde y cero throws. Quien cubre por tabla es la lente estatica del lint (cuenta las joineadas, WR-01): el switch descansa en las DOS, y el verde del sentinel solo no certifica nada
- [Phase ?]: 172-21: la sonda de la demo del fail-closed va sobre una query de UNA SOLA TABLA, y sobre un metodo que algun test EJERCITE — el getById que sugeria el plan no tiene un solo call site en src/, asi que no habria demostrado nada
- [Phase ?]: 172-21: en el switch, el conteo de entradas de allowlist del plan es un PISO y no un techo — corre el lint y lista las stale REALES antes de borrar. El plan decia 47 (y por criterio son 47 exactos), el lint reportaba 51: las 4 de diferencia eran tablas JOINEADAS que dejaron de violar como colateral de scopear la query de la tabla strict. Dejarlas deja CI rojo igual que agregarlas (D-14)
- [Phase ?]: 172-21: el orden del switch es allowlist PRIMERO y entrada strict DESPUES — al reves hay un commit intermedio con la suite roja, porque el gate D-15 se cae mientras la lista strict y la allowlist conviven. Es la misma verdad de D-15 vista desde el reloj: migrar y achicar la allowlist son un solo acto, pero si se parten en dos commits este es el unico orden verde
- [Phase ?]: 172-21: un registro critico se prueba contra una SEGUNDA COPIA escrita a mano en el test, no contra si mismo — afirmar 'finance esta presente' dejaria pasar un modulo agregado sin decidirlo; comparar el registro ENTERO contra MODULOS_DECLARADOS obliga a que sumar un modulo sea un diff en dos archivos
- [Phase ?]: 172-21: el gate de un CLI se duplica como it de la suite cuando su rojo es la unica defensa — el step de CI que corre lint:tenant esta detras del paths-filter (trampa de event.before), la suite no
- [Phase ?]: 172-21: bajar el piso numerico de un gate solo es legitimo si una asercion NUEVA contabiliza la baja unidad por unidad (87 -> 81 tablas con deuda = las 6 que entraron a strict, verificado con un filtro que exige cero tablas strict entre las violadoras). Bajar el numero pelado es la rendicion que el mensaje del gate prohibia
- [Phase ?]: 172-21: --no-file-parallelism no es una concesion al ruido ambiental, es MAS RAPIDO — test/setup.ts provisiona una base por worker (~96 s cada una), asi que con un worker se paga una vez y 33 archivos entran en 418 s. Ademas los 2 rojos 'preexistentes' de coach-load-alta pasaron asi: eran del paralelismo, no un bug de produccion
- [Phase ?]: 173-01: el backmerge D-16 (merge 395243a4) es un empalme de historia puro — los 4 commits de master ya estaban cherry-pickeados en staging y el arbol mergeado es identico byte a byte a origin/staging
- [Phase ?]: 173-01: tsconfig.test-check.json NO puede salir 0 solo (185 errores de tipos preexistentes en test/, 0 en src/, 0 TS2554) — se le agrego un ratchet con baseline versionado (pnpm typecheck:tests) copiando el idioma de lint-tenant.ts + tenant-lint-allowlist.json
- [Phase ?]: 173-01: la bateria ISO-03 de la fase son 30 rutas, no 29 — e1952606 agrego POST /api/admin/members/:userId/referrals al manifiesto; el header de tenant-manifest.ts quedo stale (dice 223/8, la realidad es 224/7)
- [Phase ?]: 173-03: GET /api/admin/leads NO EXISTE (solo PATCH /:userId) — el snapshot lo sustituye por GET /api/admin/members?status=prueba
- [Phase ?]: 173-03: linea de base D-10 capturada en $HOME/.el-templo-snapshots/173/antes.json (11 endpoints en 200, truncado=false, 0600, 11.3 MB). La foto de DESPUES tiene que reproducir la huella 57bdcd98a4a3eae4 (branchId=3 + DNI/tel del SELECT ORDER BY id LIMIT 1 sobre eltemplo_staging) o el --diff aborta con exit 2
- [Phase ?]: 173-03: un endpoint truncado ahora invalida la captura entera (exit 1 + archivo .parcial); DNI/telefono por env con solo su huella sha256 en el archivo; MAX_PAGINAS 50 -> 200 (5720 socios = 58 paginas)
- [Phase ?]: 173-03: deploy-staging.yml NO esta gateado por CI — staging desplego 395243a4 con CI rojo. Para D-10 jugo a favor: a36b759d estaba vivo antes de la foto
- [Phase ?]: 173-03: riesgo de senal declarado — export-sepa devolvio 0 filas (no hay socios ES en staging) y estado-prueba-leads solo 2: esos dos no van a cazar una regresion de conteo en el 173-31
- [Phase ?]: 173-02: el trabajo real de la fase son 364 accesos violadores en src/ (52 archivos) + 355 accesos Drizzle sin filtro en test/ (119 archivos) + 55 sitios de SQL crudo (25 archivos). Las 90 entradas de allowlist son el DIFF DE CONFIG, no el tamano del trabajo — relacion 4:1
- [Phase ?]: 173-02: las dos lentes comparten el punto ciego multi-tabla porque las DOS juzgan por STATEMENT (lint via isCompliantText(enclosingStatement) :911+:551, sentinel via analyzeSql :240). 83 statements multi-tabla, 19 YA invisibles hoy — incluido members/service.ts:370, que joinea users y branches sin tenantWhere y el lint lo marca compliant. Lectura manual obligatoria en el 173-30, no hay gate posible
- [Phase ?]: 173-02: manifiesto sincronizado — 373 entradas = 224 tenant-scoped + 141 templo-module + 8 global (commit 4d552b7d). El '372 / 7 global' del plan y de PATTERNS SS0.3 era incorrecto: una de las 8 global es OPTIONS \* (preflight de CORS), por eso 372 y 373 conviven. iso-01-manifiesto.test.ts NO estaba stale (ENTRADAS_BASELINE ya era 373): e1952606 actualizo el gate y se olvido de los 2 headers del manifiesto
- [Phase ?]: 173-02: las 30 rutas del prefijo del modulo quedan ENUMERADAS en el SUMMARY (members 24 + users 5 + leads 1) y ninguna ruta bajo esos 3 prefijos esta clasificada distinto de tenant-scoped — el gate del 173-29 puede derivar por prefijo sin excepciones
- [Phase ?]: 173-02: DOS HUECOS DE PLANIFICACION cazados cruzando violadores contra files_modified — src/scripts/backfill-historical-payments.ts:386 (viola users, fix de 1 linea, dueno propuesto 173-10) y test/fixtures/finance-gimnasio-dos.ts (dueno propuesto 173-21). Sin ellos el 173-30 sale rojo
- [Phase ?]: 173-02: TRES fugas cross-tenant concretas en members/service.ts — checkDuplicates :1743 devuelve nombre/DNI/telefono/sede de socios AJENOS por GET /members/check-duplicates (fuga de PII, no omision formal), checkDniUniqueness :1684 filtra nombre ajeno y bloquea el alta legitima en el gimnasio 2, updatePhoto :1797 escribe sin gimnasio
- [Phase ?]: 173-02: mocks posicionales = CERO sobre los 23 metodos, verificado por 4 formas (asignacion directa, vi.spyOn, vi.mock, literal de objeto). test/ nunca mockea el service de socios: siempre entra por HTTP. El cambio de firmas del 173-17 no puede romper en silencio por esa via
- [Phase ?]: 173-02: la query mas frecuente del modulo NO esta en members/ sino en country-scope.ts (11 de 31 violaciones en un solo barrido, corre en cada request admin) — mejor candidata para la demo del fail-closed en vivo
- [Phase ?]: 173-02: el grep de SQL crudo prescrito (con backticks) veia 3 de 55 sitios — en este arbol el idioma es sql`DELETE FROM audit_log` SIN backticks. audit_log strict rompe 11 beforeEach (10 en el 173-23, 1 en el 173-22)
- [Phase ?]: 173-02: test/helpers.ts:289-293 dejo una decision escrita a nombre de esta fase sobre los 2 statements de users de cleanAllTestData. La toma el 173-21 — recomendacion EXENCION (isolate: false obliga al borrado global), con el motivo textual propuesto en el SUMMARY
- [Phase ?]: 173-02: ADO-02 NO se marca completo por un plan de inventario — el frontmatter del 173-02 lo declaraba y el handler lo marco; se revirtio a mano. ADO-02 ('members migrado al patron completo') es cierto recien despues del switch del 173-30. Revisar el mismo riesgo en el frontmatter de los otros planes de inventario/preparacion de la fase
- [Phase ?]: 173-02: la lente B no se corrio como pedia el plan (suite entera, prohibida local, ~106 s por archivo) sino con un arnes desechable (buildApp + inject sobre 18 endpoints del prefijo + dbSentinel.report()), creado, corrido y borrado. Cubre el modulo, NO los 140 archivos: el inventario total sale gratis en CI, no local
- [Phase 173]: 173-23: recomputeUserStatus/activateScheduledSub (subscriptions/service.ts) recibieron guard isNotNull(tenantId) en el camino ctx=null — bloqueaban cualquier lectura de suscripcion con users strict; deuda ADO-07/fase-174 intacta — Rule 3 blocking, deuda ya documentada por 173-14, cirugia minima sin ensanchar D-02
- [Phase 173]: 173-23: reports/service.ts (getExpiredMembers, getExpiringMemberships, getInactiveMembers, getTrialSessionsReport, loadRosterAttribution) migrados con ctx real — cada uno con un unico call site en reports/routes.ts — Rule 3 blocking, deuda documentada por 173-05; colateral positivo para la allowlist que 173-30 deberia verificar
- [Phase 177]: 0205 es el numero libre real de migracion (177-01) -- verificado contra el directorio de migraciones en TODAS las ramas locales, no solo la de trabajo (maximo previo 0204 en et-159 sin pushear)
- [Phase 177]: categoryGroup('paquete') === 'presencial' e isOnlinePlan('paquete') === false (177-01) -- paquete conflictua con presencial, coexiste con online, resuelto correcto desde el dia 1
- [Phase 177]: Gap conocido documentado (177-01, NO corregido): sameGroupCategoryCondition en assignPlan usa eq literal 'presencial' en vez de categoryGroup -- no detecta presencial-existente vs paquete-nuevo ni paquete vs paquete; fuera de alcance por restriccion dura de Franco de no tocar la logica de asignacion de assignPlan esta fase. Revisar antes de 177-03/04.
- [Phase 177]: D-13 exclusión de paquete de Aura+referidos: predicados DRY excludedFromReferrals/excludedFromAura en types.ts, aplicados en los 9 callsites de composición de precio de service.ts (177-02) — Un descuento Aura/referido sobre un paquete de fórmula cerrada rompería el guardrail de no-canibalización
- [Phase 177]: 177-03: tab paquete resuelve planId sin motor de precios en el front (find sobre plans.value, precio siempre de priceRegular) — El país ya viene acotado por branchId en getPlans; sin combinación válida se limpia selectedPlan reutilizando el guard existente
- [Phase ?]: Toggle 'Paquete de clases' en AssignPlanDialog visible sólo para catálogo presencial (sin categoryFilter u 'presencial'), sede física, y con filas paquete cargadas
- [Phase ?]: resolvedPaquetePlan busca en plans.value crudo (no filteredPlans, que ahora excluye paquete siempre) — mismo patrón que CobrosPage 177-03
- [Phase ?]: watch([resolvedPaquetePlan, paqueteMode]) dispara selectPlan(plan) completo (no solo asigna selectedPlan) para reutilizar reset de assignForm y change-preview en mode='change' (D-11)
- [Phase 177]: 177-05: matriz de paquete insertada por el propio test (no lookup de fila seeded por migracion) porque cleanAllTestData borra subscription_plans en cada beforeEach y los workers de vitest son compartidos entre archivos (isolate:false)
- [Phase 177]: 177-05: enforce del tope semanal confirmado por lectura de codigo (booking-service.ts:294-311) y test end-to-end -- isBonus solo exime en planes bookingMode=fixed, paquete es flexible
