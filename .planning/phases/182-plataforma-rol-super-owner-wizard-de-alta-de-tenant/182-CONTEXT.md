...
- **D-11: El wizard crea el primer owner del gimnasio:** un `users` con rol `owner` en el tenant nuevo, a partir del email del dueño, con **contraseña definitiva fijada de una vez en el wizard** (el super-owner la escribe o la genera con un botón; se muestra una sola vez, copiable, en la pantalla final). **SIN cambio de contraseña forzado al primer login** — decisión del usuario tras el research (C-2: `users` no tiene esa columna y agregarla tocaba el login de El Templo, prohibido por D-05). El owner la cambia si quiere desde el flujo `change-password` existente. **Sin depender del envío de mails.**
...
- **D-17: Tenant `demo` de plataforma en producción** durante la fase, para UAT real de subdominio, login del owner y aislamiento. Slug tipo `demo` (**NO** agregarlo a `RESERVED_TENANT_SLUGS`: el wizard lo rechazaría — research C-5); queda como tenant permanente de demostración; los crons `forEachActiveTenant` lo iteran pero con módulos Templo OFF no producen nada. Se archiva desde el panel de la 183 si molesta. **Staging primero, luego prod (decisión post-research):** el alta `demo` se hace primero en staging (UAT del wizard con su propio demo) y, aprobado, se repite en prod por el mismo wizard; staging y prod comparten host MySQL, así que antes de cada alta se verifica qué base usa el entorno.

### Decisiones post-research (2026-08-28, respondidas por el usuario tras 182-RESEARCH.md)

- **D-18: Tenant pre-login por `Origin` / header `X-Tenant-Slug`, NO por hostname de la API.** La API sigue en un solo hostname (`api.eltemplo.org` / `api-staging.eltemplo.org`); no hay hostnames de API por tenant ni vhost regex nuevo para la API. Para los requests **sin usuario** que vienen de un gimnasio nuevo (login, branding público), la API resuelve el tenant así: (1) si el request trae `Origin` cuyo hostname cae **exactamente** bajo `PLATFORM_DOMAIN` (regex anclada, un label `<slug>.<PLATFORM_DOMAIN>` o `<slug>.staging.<PLATFORM_DOMAIN>` según entorno) → lookup exacto de `<slug>` contra `tenants.slug`; (2) si no hay `Origin` de plataforma pero viene header `X-Tenant-Slug` (app nativa Kaia, un interceptor de una línea) → mismo lookup; (3) cualquier otro caso → la API se comporta exactamente como hoy (D-05). Slug no resoluble → 404 `TENANT_NOT_FOUND` (D-04). Los requests **autenticados** siguen derivando el tenant del `tenant_id` del usuario, sin cambios. Esto **reinterpreta H-3 del doc 08** (hostname del front en vez de hostname de la API) y la fase debe dejar la enmienda anotada en `08-diseno-modulo-gimnasio.md` §H-3 (párrafo corto, sin reescribir la sección). D-02 se reduce a: vhost wildcard de nginx + cert DNS-01 **solo para el front** (`*.<PLATFORM_DOMAIN>` y `*.staging.<PLATFORM_DOMAIN>`), CORS dinámico en la API.
- **D-19:** ver D-11 enmendada (contraseña definitiva en el wizard, sin forzado) y D-17 enmendada (demo staging-first).
- **Nota de alcance (research C-3):** no existe pantalla ni API de ABM de sedes en el admin; D-12 la daba por existente. El paso opcional "primera sede física" del wizard es el único camino de alta de sedes para el tenant nuevo en esta fase; el ABM de sedes queda **diferido** (183/192 o housekeeping) y se registra en Deferred.
- **Nota (research, D-15):** NO tocar `MODULE_NAMES` en esta fase — agregar `"gimnasio"` rompe `mod-01-flags` y enciende gimnasio para El Templo en tests. La fila `module.gimnasio.enabled = "true"` se siembra como string literal (inerte para `parseModuleFlagKey`, auditable con SELECT); la entrada al enum va en la 184 con el rename.
...
- **Login scoped por host para la app Kaia** — el endpoint completo pertenece a la fase de la app de alumnos; la 182 deja la resolución por `Origin`/`X-Tenant-Slug` (D-18) lista para consumirlo.
- **ABM de sedes en el admin** (crear/editar sedes después del alta) — no existe (research C-3); diferido a 183/192 o housekeeping. En la 182 solo la sede física opcional del wizard.
- **Cambio de contraseña forzado al primer login del owner** — descartado por el usuario (D-11 enmendada); si algún día se quiere, es una columna aditiva en `users` + campo en la respuesta del login.
- **Hostname de API por tenant (`api-<slug>.<PLATFORM_DOMAIN>`)** — descartado en favor de D-18.
