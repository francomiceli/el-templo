...
**TAKE-01 — Context-aware escalation ACK (LLM-driven, one-shot at takeover moment):**

- The current static `HANDOFF_ESCALATION_PHRASE` MUST become context-aware, referencing the `request_human` `reason` arg so the lead sees the bot acknowledged their specific situation before going silent.
- Implementation choice (planner picks based on existing pattern fit; both are acceptable):
  - **Sub-option A (recommended — LLM-driven via system prompt):** When the escalation turn runs the AI (which it already does — the escalation message is generated as part of the request_human tool-call cycle), add a system-prompt addendum (analogous to Phase 99's `PB1_PRICE_DISCLOSURE_UNLOCKED_ADDENDUM`) instructing the model to emit a context-aware handoff: _"vi que estás lesionado, te paso con alguien del equipo que te asesore bien"_ for injury reasons, etc. Keep the static `HANDOFF_ESCALATION_PHRASE` as a fallback for code paths that don't run the AI (auto-escalation-after-2-failures may be one such path).
  - **Sub-option B (template-style):** Refactor `HANDOFF_ESCALATION_PHRASE` from a constant to a function `getHandoffPhrase(reason: string): string` with a reason→phrase mapping for the known reason values (`auto_escalation_after_2_failures`, `injury`, `objection_unresolved`, etc.) plus a default. Simpler but less natural language.
- Planner picks based on whether the escalation path currently goes through the LLM consistently (Sub-option A) or has non-AI branches that need static fallbacks (Sub-option B). Document the choice in the plan.
...
- **TAKE-01** — Context-aware escalation ACK at the moment of human-takeover (LLM-driven via system-prompt addendum OR per-reason handler function — planner picks Sub-option A or B based on existing-pattern fit); references `request_human` reason arg
