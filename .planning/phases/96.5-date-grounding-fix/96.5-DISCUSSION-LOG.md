...
2. **Filesystem naming convention** — locked to **period throughout for fractional phases** (`96.5-CONTEXT.md`, `96.5-DISCUSSION-LOG.md`, directory `96.5-date-grounding-fix/`); matches SDK `padded_phase: "96.5"` and `expected_phase_dir` heuristic. Convention initially explored with hyphen separators (`96-5-`) before SDK verification identified period as the canonical pattern for fractional phases. Reverted in this session's rename commit.
