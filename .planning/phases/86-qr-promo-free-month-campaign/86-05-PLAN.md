...
Inside the `subscriptionRoutes` function, add these endpoints. Use the same auth guard pattern as existing routes (`onRequest: [fastify.authenticate]` + role check):
...
async function listPromos(): Promise<PromoListItem[]> {
  const { data } = await api.get("/subscriptions/promo-plans");
  return data;
}

async function createPromo(input: CreatePromoInput): Promise<PromoListItem> {
  const { data } = await api.post("/subscriptions/promo-plans", input);
  return data;
}

async function deactivatePromo(promoId: number): Promise<void> {
  await api.patch(`/subscriptions/promo-plans/${promoId}/deactivate`);
}
```
