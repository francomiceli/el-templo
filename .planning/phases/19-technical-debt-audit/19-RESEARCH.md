...
| aws-cli (system) | v2 | Upload backups to S3/B2 | Both S3 and Backblaze B2 support S3-compatible API via aws cli |
...
| Backblaze B2 | AWS S3 | B2 is ~1/4 the cost ($6/TB vs $23/TB), S3-compatible API, use aws cli with endpoint-url |
...
RETENTION_DAYS=7
BUCKET="s3://eltemplo-backups"  # Works with both S3 and B2 via endpoint-url
...
if command -v aws &> /dev/null; then
  aws s3 cp "$BACKUP_FILE" "${BUCKET}/" --endpoint-url "${B2_ENDPOINT_URL:-}"
...
| S3-compatible upload | Custom HTTP upload to B2/S3 | aws cli with --endpoint-url | Handles multipart, retry, checksums; works with both S3 and B2 |
...
### Cloud Storage: Backblaze B2 (MEDIUM confidence)
**Recommendation:** Backblaze B2 is approximately 1/4 the cost of AWS S3 ($6/TB/month vs $23/TB/month) and provides an S3-compatible API. The aws CLI works with B2 via `--endpoint-url`. For a solo developer with small backup sizes (<1GB), the cost difference is negligible, but B2's simpler pricing (no request charges, free egress up to 3x storage) makes it easier to understand. Either works; B2 is slightly more cost-effective.
...
| Separate S3 and B2 CLIs | aws cli with `--endpoint-url` for B2 | B2 S3-compatible API (2022+) | Single tool for both providers |
...
- Backblaze B2 S3-compatible API: https://www.backblaze.com/docs/cloud-storage-use-the-aws-cli-with-backblaze-b2
