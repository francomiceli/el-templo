# Requirements Archive: v6.0 Tenancy — El Templo pasa a ser tenant #1
...
- [x] **ADO-02**: `members` ídem
- [x] **ADO-03**: `subscriptions` ídem, con la cadena de pricing (override → boarding pass → AURA → referral) intacta
- [x] **ADO-04**: `scheduling` ídem (schedules/bookings/attendance/schedule_exceptions)
- [x] **ADO-05**: `analytics` ídem
- [x] **ADO-06**: Resto del core ídem (campaigns, notifications, referrals, wellhub, feedback/improvement_proposals, auth/settings) — incluye supresión de unsubscribes POR TENANT (decisión Q5, mina M3)
- [x] **ADO-07**: Guarda de consistencia `user.tenant_id === branch.tenant_id` en los ~10 sitios de escritura de `branch_id` + `setMemberBranch()` + cron de recategorización (mina M10)

### MOD — Mecanismo de módulos

- [x] **MOD-01**: Flags `module.<nombre>.enabled` en `tenant_settings` + guard `requireModule` (404) gatean las rutas de los 4 módulos Templo (templo-training/gamification/marketing/onboarding) — prendidos para tenant 1, apagados por default para tenants nuevos
- [x] **MOD-02**: Registry de hooks tipado con la superficie mínima validada (doc 04): filter `pricing.adjust` (bloqueante) + event `streak.milestone` (best-effort), composition root explícito
...
| ADO-02  | Phase 173 | Complete |
| ADO-07  | Phase 173 | Complete |
| ADO-03  | Phase 174 | Complete |
| ADO-04  | Phase 174 | Complete |
| ADO-05  | Phase 175 | Complete |
| ADO-06  | Phase 175 | Complete |
| MOD-01  | Phase 176 | Complete |
| MOD-02  | Phase 176 | Complete |
...
- **Cierre 2026-08-26:** las 10 filas que quedaron `Pending` por lag de docs (CON-05/06,
  ADO-02→07, MOD-01/02) se marcaron Complete al cierre del milestone con evidencia verificada:
  sentinel + lint CON-06 corriendo como gates de CI y en prod (modo log, 0 fallos internos),
  switches strict de members/subs/scheduling/analytics/resto ejecutados (fases 173→175.1,
  batería ISO-03 644/644), módulos MOD-01/02 con mod-02 11/11 (fase 176). D2 definitivo:
  `db:verify-tenant` EXIT 0 con 0 filas de tenant ≠ 1 en `eltemplo_staging` Y `eltemplo` (prod).
