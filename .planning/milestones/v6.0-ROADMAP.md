...
## v5.6 (Semana nueva combos + técnica) Overview — Phases 159-160

**Milestone:** v5.6 — Semana nueva de entrenamiento: días de combos y técnica como modos de primera clase, de punta a punta (generador, edición, horarios, app, PDF, TV)

**Status:** ACTIVO (re-especificado 2026-08-13 por el usuario). La planificación original (2026-07-07) quedó en la rama muerta `feat/dias-combos-tecnica` (commit `5b153783`) y se reconstruye acá con alcance ampliado (horarios + TV) y varias decisiones cerradas.

**Contexto:** En prod, el régimen combos/técnica existe desde la **semana 12** (~fines de abril 2026) hackeado a mano sobre `regular`. **⚠️ Corrección 2026-08-13 (SSH read-only, DB `eltemplo`): el régimen ALTERNA por decisión del profe, NO está fijo.** El formato del NUCLEUS por semana lo prueba: W21 mié=Combos/jue=técnica, W23 igual, pero W24/W25/W26 fueron mié=técnica/jue=combos. La conclusión previa de "fijo mié=combos desde W19" quedó DESMENTIDA. Por eso combos/técnica son **modos que el profe elige por día en `/generate`**, no un mapeo fijo por día de la semana. Combos hoy: `{"type":"combos","rounds":""}` + reps por ejercicio. 6 niveles reales (alfa, delta, kairos, omega, spartan, sigma) en 3 `level_group` (alfa_delta, omega, sigma). Bloque final actual = EPIKOS (Flow Guiado/Circuito cooperativo) → lo reemplaza STRETCHING. Viernes "open" sin confirmar (**fuera de alcance; queda `regular`**). Discovery técnico: `.docs/coach-improvements/DISCOVERY-SEMANA-NUEVA-2026-07-07.md`. Fuentes de negocio: `.docs/product/evoluicion-producto.md` (abril 2026) + `COACH_IMPROVEMENTS_HANDOFF_2026-06-21.md`. **Ya en prod desde 2026-07-09 (migs 0172-0173):** formato "Combos", formato "Stretching", ruta FB — esta fase construye sobre eso. **Base de rama: `master` actual (v6.0 NO va a master todavía).**

**Spec del usuario (2026-08-13) — estructura de los días, CERRADA:**

- **Modo elegido por el profe en `/generate`** (regular/rom/combos/tecnica), por día — NO atado a mié/jue fijo. `day_modes` queda solo como default de ROM.
- **Día de Combos:** INITIUM → **COMBOS I** (reemplaza a NUCLEUS, tren superior) → **COMBOS II** (reemplaza al bloque a elección DEUTEROS, tren inferior — NO hay bloque a elección) → **STRETCHING**.
- **Día de Técnica:** INITIUM → **TECNICA I** → **TECNICA II** (I y II sobre la **misma ruta** para afianzar el aprendizaje, ej. ambos planche) → **STRETCHING**.
- **El bloque final de ambos días se llama "STRETCHING"** — CERRADO (DIATASIS/EKTASIS descartados). Reutiliza el pool de movilidad de ROM, nivel único, generador elige 4 y el profe edita.
- **Rename: las opciones de DEUTEROS pasan de "I"/"II" a "A"/"B"** en los días regulares.
- **Etiqueta en horarios/app derivada de la sesión generada del día** (combos→"Combos", tecnica→"Técnica", regular→"General"). Rename "Calistenia"→"General". Solo etiqueta, sin tocar reservas/cupos.
- **Superficies:** (1) `/generate` (sumar combos/tecnica a las opciones de tipo de sesión) + edición admin; (2) **horarios** (nombre derivado); (3) member app; (4) PDF; (5) **TV de sucursal**.

### Phase 159: Semana nueva backend — modos de día, generadores, roles de bloque y horarios

**Goal:** El generador produce días de combos y de técnica como modos de primera clase (análogos a `rom`), **elegidos por el profe día a día en `/generate`**, con la estructura de bloques cerrada arriba, sin edición manual masiva del coach; y las clases muestran su nombre derivado de la sesión generada. End state: `sessions.session_mode` acepta `combos` y `tecnica`; el backend de `/generate` acepta el modo por día; existen los roles de bloque COMBOS I/II, TECNICA I/II y STRETCHING; `combos-generator` y `tecnica-generator` cubren los 6 niveles; las opciones de DEUTEROS se renombran A/B; y existe el ancla histórica semana→fecha/régimen (histórico intacto).
**Depends on:** none. **Base: `master` actual** (decisión del usuario; v6.0 no va a master aún). **Migraciones a verificar en plan-phase:** tope aplicado en prod = 0201; verificar el máximo en master antes de generar SQL (nacen en 0202+, salvo colisión con features en vuelo a master). La rama vieja `feat/dias-combos-tecnica` NO se rebasea.
**Requirements:** SEM-01 (`session_mode` acepta `combos`/`tecnica`, migración aditiva, histórico intacto), SEM-02 (formato Combos: se mantiene la forma actual de reps del profe — rounds + reps por ejercicio; NO parámetro único nuevo), SEM-03 (combos-generator: INITIUM → COMBOS I superior → COMBOS II inferior → STRETCHING), SEM-04 (tecnica-generator: INITIUM → TECNICA I → TECNICA II misma ruta → STRETCHING), SEM-05 (ancla semana→fecha/régimen + retro-etiquetado W12+ con las firmas del discovery, metadata sin tocar filas históricas), SEM-06 (tests de integración de ambos generadores), SEM-12 (roles de bloque nuevos COMBOS I/II, TECNICA I/II, STRETCHING + rename opciones DEUTEROS I/II → A/B, aditivo), SEM-13 (`/generate` acepta el modo combos/tecnica por día en el backend; etiqueta de clase derivada de la sesión generada; rename "Calistenia"→"General").
**Decisiones cerradas en discuss-phase (2026-08-13):** ver `159-CONTEXT.md` — selección por día en `/generate` (no day_modes fijo); COMBOS I superior/II inferior; TECNICA I+II misma ruta; STRETCHING reutiliza pool ROM (nivel único, generador elige 4); reps del combo = forma actual del profe; 6 niveles; etiqueta horarios derivada de la sesión. Pendiente para plan-phase: numeración de migraciones en master.
**Plans:** 6 plans in 4 waves

- [ ] 159-01-PLAN.md — Wave 1: tipos (BlockRole+5, sessionMode+2), validadores exhaustivos, body de /generate (dayModes + enum + additionalProperties) (SEM-01, SEM-12)
- [ ] 159-02-PLAN.md — Wave 2: tronco compartido (semana-nueva-pipeline con Stage 1 reemplazado + stretching-selection determinística) (SEM-03, SEM-04, SEM-06)
- [ ] 159-03-PLAN.md — Wave 3: combos-generator + tecnica-generator + tests unitarios (SEM-02, SEM-03, SEM-04, SEM-06)
- [ ] 159-04-PLAN.md — Wave 1: ancla session_week_regime (schema + DDL 0202 + backfill 0203 W12-W26, histórico intacto) (SEM-05)
- [ ] 159-05-PLAN.md — Wave 4: ruteo por modo en generateWeek + badge DEUTEROS DA/DB + test de integración de /generate (SEM-01, SEM-12, SEM-13)
- [ ] 159-06-PLAN.md — Wave 1: etiqueta de clase derivada + rename Calistenia→General (mig 0204) + fix TV class-day + test (SEM-13)

### Phase 160: Semana nueva frontend — admin UI, horarios, app, PDF y TV

**Goal:** El coach genera y edita la semana nueva desde el admin sin hacks, y los días de combos/técnica se ven con sus nombres correctos en todas las superficies. End state: GeneratePage ofrece los modos `combos`/`tecnica`; SessionsPage los señaliza; FormatParamsEditor edita el `reps` del combo; los horarios muestran las clases de mié/jue con su nombre; el PDF renderiza los dos tipos de día (incl. STRETCHING); la member app (BlockCard/player) los muestra; la TV de sucursal los renderiza; los labels de roles de bloque quedan centralizados (hoy duplicados en ≥4 archivos: `BlockCard.vue`, `GoalPlanSession.vue`, `useGoalPlanSession.ts`, `session-pdf-builder.ts`) con DEUTEROS A/B incluido.
**Depends on:** Phase 159 (modos, generadores, roles y modelado de horarios en el backend).
**Requirements:** SEM-07 (GeneratePage: sumar combos/tecnica al selector de tipo de sesión por día + badges SessionsPage), SEM-08 (editor de bloques Combos con la forma de reps actual), SEM-09 (PDF de combos y técnica con STRETCHING), SEM-10 (member app render), SEM-11 (labels de roles centralizados, incl. rename A/B en todas las superficies), SEM-14 (horarios/app: nombre de clase derivado de la sesión generada — Combos/Técnica/General), SEM-15 (TV de sucursal renderiza los días nuevos — coordinar con el trabajo TV vivo en `et-tv2`/`feat/tv-login-staging`).
**Plans:** TBD

---

_v5.6 (Semana nueva combos + técnica) re-especificado: 2026-08-13 — 2 phases (159-160), 15 requirements (SEM-01..15). Discuss-phase 159 hecho el 2026-08-13 con SSH read-only a prod: **hallazgo clave — el régimen combos/técnica ALTERNA por decisión del profe (probado W21-W26), NO es fijo**; por eso el modelo es **selección por día en `/generate`** (no day_modes fijo), etiqueta de horarios derivada de la sesión generada. Spec de días CERRADA (COMBOS I sup/II inf, TECNICA I+II misma ruta, STRETCHING reutiliza pool ROM, DEUTEROS A/B, 6 niveles, reps = forma actual del profe). **Base = master** (v6.0 no va a master aún); migraciones 0202+ a verificar en plan-phase. Viernes y Full Body fuera de alcance (FB ya en prod). La rama vieja `feat/dias-combos-tecnica` NO se rebasea. Detalle: `159-CONTEXT.md`. Fuentes: `evoluicion-producto.md` + `DISCOVERY-SEMANA-NUEVA-2026-07-07.md` + `COACH_IMPROVEMENTS_HANDOFF_2026-06-21.md` + evidencia SSH prod + spec del usuario 2026-08-13._
...
- [ ] **Phase 174: Adopción 3a — `subscriptions` + `scheduling`: adopción de código** — planes/suscripciones (cadena de pricing intacta) y scheduling completo migrados a nivel código, más las deudas heredadas de la 173; sin switch.
- [ ] **Phase 174.1: Adopción 3b — batería ISO y switch de `subscriptions` + `scheduling`** — fixtures del gimnasio 2, baterías de aislamiento, gate de cobertura, allowlist saldada y ambos módulos en strict (patrón 173-26..30).
- [ ] **Phase 175: Adopción 4a — `analytics` + resto del core: adopción de código** — métricas scopeadas con los mismos números para el tenant 1; campaigns/notifications/referrals/wellhub/sugerencias/auth completan las deudas de la 173; unsubscribes por tenant.
- [ ] **Phase 175.1: Adopción 4b — batería ISO, switch final y cierre de gates del milestone** — core 100% aislado y en strict, gate de salida del milestone, REQUIREMENTS familia ISO al día.
...
- [x] 173-04-PLAN.md — helpers compartidos: `audit-log.ts` y `country-scope.ts`
- [x] 173-10-PLAN.md — retrofit `requireTenant` en los 6 scripts CLI (D-03)
- [x] 173-11-PLAN.md — helper de anclas `branch-consistency.ts` y `canAccessBranch` por gimnasio (D-05a, D-14)

**Wave 3** _(blocked on Wave 2 completion)_

- [x] 173-05-PLAN.md — helpers compartidos: `member-search.ts` y `covered-until.ts`
- [x] 173-06-PLAN.md — cirugía mínima en los 6 services de analytics
- [x] 173-08-PLAN.md — cirugía mínima en campañas, segmentación, notificaciones, onboarding y referidos
- [x] 173-09-PLAN.md — cirugía mínima en entrenamiento, progresión y gamificación
- [x] 173-12-PLAN.md — schema + migración 0198: unique `(tenant_id, id)` y FK compuesta (D-05b, D-18)
- [x] 173-15-PLAN.md — WR-01 del autorregistro (D-12) y Wellhub

**Wave 4** _(blocked on Wave 3 completion)_

- [x] 173-13-PLAN.md — `users/service.ts` y `users/routes.ts`: staff, `user_branches` y 6 sitios de ancla
- [x] 173-16-PLAN.md — los 3 crons y el test SC3 del cron de recategorización (D-04, D-07)
- [x] 173-17-PLAN.md — plomería de firmas: `ctx` primero en los 23 métodos y `assertTenant` en 25 rutas

**Wave 5** _(blocked on Wave 4 completion)_

- [x] 173-07-PLAN.md — cirugía mínima en scheduling, asistencia, check-ins y ratings
- [x] 173-14-PLAN.md — deudas de la 172 en `subscriptions/service.ts` (D-13, D-15) y desmarque del ancla
- [x] 173-18-PLAN.md — queries de escritura de `members/service.ts` y los 4 sitios de ancla
- [x] 173-20-PLAN.md — queries directas de `members/routes.ts` y `leads-routes.ts`

**Wave 6** _(blocked on Wave 5 completion)_

- [x] 173-19-PLAN.md — queries de lectura de `members/service.ts` y las 11 entradas por archivo

**Wave 7** _(blocked on Wave 6 completion)_

- [x] 173-21-PLAN.md — endurecimiento de test/: infra, `cleanAllTestData` y la sonda

**Wave 8** _(blocked on Wave 7 completion)_

- [x] 173-22-PLAN.md — endurecimiento de test/: socios, staff, auth y leads

**Wave 9** _(blocked on Wave 8 completion)_

- [x] 173-23-PLAN.md — endurecimiento de test/: suscripciones, finanzas, reportes, coach y referidos
...
### Phase 174: Adopción 3a — `subscriptions` + `scheduling`: adopción de código

**Goal:** Los dos módulos que mueven la operación diaria —planes/suscripciones y grilla/reservas/asistencia— quedan migrados a nivel CÓDIGO (ctx + tenantWhere/tenantValues + guardas de ancla), con la cadena de pricing intacta. El switch a strict y la batería de aislamiento son de la fase 174.1 — esta fase NO enciende el sentinel. End state: asignar, renovar, cambiar plan, reservar, cancelar y marcar asistencia funcionan idénticamente para el Templo, con el código listo para aislar.
...
2. `scheduling` migrado al patrón completo incluyendo el booking de admin, el check-in por QR y las excepciones de fecha. (ADO-04)
3. Las deudas que la 173 dejó dirigidas a esta fase (deferred-items de la 173) quedan saldadas: los guards `isNotNull` provisorios del camino member-facing (`autoExpireSubscriptions` con ctx=null), los inserts sin `tenantValues` (`program_enrollments` y los sitios listados) y `activeMemberExists` con ctx real.
...
- [ ] 174-01-PLAN.md — subscriptions: lecturas + lifecycle no-pricing (service.ts, booking-population.ts, member-routes.ts)
- [ ] 174-03-PLAN.md — scheduling: booking-service.ts (mayor superficie cruda + holidays)
- [ ] 174-04-PLAN.md — scheduling: service.ts + trials/activity/holiday/schedule-exceptions + routes wiring
- [ ] 174-05-PLAN.md — attendance: service.ts (reads + los 5 inserts)

**Wave 2** _(blocked on Wave 1 completion)_

- [ ] 174-02-PLAN.md — subscriptions: cadena de pricing con golden tests (diff cero)

**Wave 3** _(blocked on Wave 2 completion)_

- [ ] 174-06-PLAN.md — cierre de deudas 173: ctx real D-07 + guarda de ancla + excepción cron + program_enrollments

### Phase 174.1: Adopción 3b — batería ISO y switch de `subscriptions` + `scheduling` (INSERTED)

**Goal:** Los dos módulos quedan AISLADOS y con el sentinel en throw: batería de aislamiento completa, allowlist saldada y entrada en `TENANT_STRICT_MODULES`. Réplica del patrón 173-26..30 (fixtures del gimnasio 2 → baterías por grupo de rutas → gate de cobertura → allowlist PRIMERO, strict DESPUÉS).
...
1. Batería de aislamiento verde para todas las rutas `tenant-scoped` de ambos módulos: staff del tenant A no ve grillas, cupos, reservas, asistencias ni suscripciones del tenant B, ni puede reservar contra un horario ajeno; gate de cobertura derivado del manifiesto (patrón 172-20/173-29). (ADO-03, ADO-04)
2. Allowlist sin una sola entrada de los archivos de ambos módulos; `subscriptions` y `scheduling` entran a `TENANT_STRICT_MODULES` (orden de commits: allowlist primero, strict después); `con-06-lint` y el gate de forma verdes.
3. Suite completa verde con el switch dado: 0 throws del sentinel en test/dev.

**Plans:** 10 plans

Plans:

- [ ] 174.1-01-PLAN.md — Fixture gym-2 (subs+sched) + bateria ISO lecturas de suscripciones
- [ ] 174.1-02-PLAN.md — removeCheckIn: ctx real + assertTenant (prerequisito D-04)
- [ ] 174.1-03-PLAN.md — Migracion analytics (30 accesos) a tenantWhere
- [ ] 174.1-04-PLAN.md — Migracion modulos con actor real a tenantWhere
- [ ] 174.1-05-PLAN.md — Migracion crons/imports/wellhub (tenant-safe / tenantWhere)
- [ ] 174.1-06-PLAN.md — Bateria ISO escritura de suscripciones (lifecycle + CRUD)
- [ ] 174.1-07-PLAN.md — Bateria ISO lecturas de scheduling
- [ ] 174.1-08-PLAN.md — Bateria ISO escritura de scheduling (reservar contra ajeno)
- [ ] 174.1-09-PLAN.md — Gate de cobertura fail-closed (4 prefijos)
- [ ] 174.1-10-PLAN.md — El switch: allowlist + strict + fail-closed en vivo

### Phase 175: Adopción 4a — `analytics` + resto del core: adopción de código

**Goal:** Se cierra la adopción de CÓDIGO del core: las métricas de gestión filtran por tenant devolviendo los mismos números para el Templo, y los módulos restantes (campañas, notificaciones, referidos, wellhub, sugerencias, auth/settings) completan lo que la 173 dejó a medias (gran parte de su código ya fue scopeado por los planes 173-06/08/15). El switch y la batería son de la 175.1.

**Depends on:** Phase 174.1 (analytics lee de finanzas, socios, suscripciones y asistencia — se migra después de sus fuentes). Toca `el-templo-api` (analytics/reportes; `campaigns` + `campaign_sends`/`events`/`unsubscribes`; notificaciones y `device_tokens`; `referrals`/`referral_credits`/`referral_cta_clicks`; wellhub; `improvement_proposals`; auth y settings).
...
2. Campaigns, notifications, referrals, wellhub, feedback/sugerencias y auth/settings completan el patrón: se saldan las deudas listadas en los deferred-items de la 173 (`getReferralOverview` y `generateReferralCode` con ctx real, guards `isNotNull` restantes, inserts sin `tenantValues` de estos módulos). (ADO-06)
3. La supresión de unsubscribes es **por tenant** punta a punta: un opt-out registrado en el tenant A no suprime ningún envío del tenant B, verificado con un envío end-to-end de prueba (cierra la mina M3 del lado del comportamiento) — incluye verificar que campaigns esté listo para uso por-tenant (salvedad de Franco en el checkpoint 171-06). (ADO-06)
4. Sin cambio de números ni de comportamiento para el tenant 1 (suite existente verde sin ajustar expectativas).

**Plans:** TBD

### Phase 175.1: Adopción 4b — batería ISO, switch final y cierre de gates del milestone (INSERTED)

**Goal:** El core queda 100% aislado y el milestone cierra sus gates: baterías y switch de analytics y los módulos restantes, gate de salida del milestone (manifiesto sin rutas core `tenant-scoped` fuera de la batería) y puesta al día de REQUIREMENTS para toda la familia ISO (deuda contable arrastrada desde la 171/172).

**Depends on:** Phase 175

**Requirements:** ADO-05, ADO-06, ISO-01, ISO-02, ISO-03

**Success Criteria** (what must be TRUE):

1. Batería de aislamiento verde para todas las rutas `tenant-scoped` de analytics y de los módulos restantes; el manifiesto no tiene NINGUNA ruta core `tenant-scoped` fuera de la batería (insumo directo del gate de salida del milestone). (ADO-05, ADO-06)
2. Los módulos restantes entran a `TENANT_STRICT_MODULES` (allowlist saldada primero); suite completa verde con todo el core en strict, 0 throws.
3. `REQUIREMENTS.md` al día para toda la familia ISO (ISO-01/ISO-02/ISO-03 reflejan la realidad — deuda de cierre anotada desde la 172), en un solo pase.
...
**Depends on:** Phase 175.1 (todo el core migrado y en strict; el registry engancha el pricing ya scopeado). Toca `el-templo-api` (`tenant_settings`, guard `requireModule`, registry de hooks, composition root, rutas de los módulos templo-training/gamification/marketing/onboarding).
...
- [ ] 176-01-PLAN.md — Worktree de la fase, `modules.ts`/`module-flags.ts` (resolver fail-closed con cache TTL) y migración 0207 con seed idempotente de los 4 flags ON para tenant 1

**Wave 2** _(blocked on Wave 1 completion)_

- [ ] 176-02-PLAN.md — Baseline sellado del golden de pricing + cierre de los dos huecos medidos (ES/EUR y los 4 tiers de AURA en `changePlanAfterCurrent`)
- [ ] 176-03-PLAN.md — Guard `requireModule` (404) + `moduleScope` (`onRoute`-append) y los 12 registros de `templo-training` envueltos in-place en `app.ts`

**Wave 3** _(blocked on Wave 2 completion)_

- [ ] 176-04-PLAN.md — Composition root `src/modules-boot.ts` con los `ModuleDef` de marketing (5), onboarding (1) y gamification (1)

**Wave 4** _(blocked on Wave 3 completion)_

- [ ] 176-05-PLAN.md — Gate bidireccional en ISO-01: cobertura 141/141 + no-derrame, e inversión de la lista de módulos hacia `src/`
- [ ] 176-06-PLAN.md — `HookRegistry` tipado (filters propagan, events aíslan, módulo OFF no corre) colgado del composition root

**Wave 5** _(blocked on Wave 4 completion)_

- [ ] 176-07-PLAN.md — Event `streak.milestone`: la recompensa AURA por racha se va al módulo y `StreakService` pierde `AuraService`

**Wave 6** _(blocked on Wave 5 completion)_

- [ ] 176-08-PLAN.md — `pricing.ts` (función core que llama al filter) + handler `pricing.adjust` del módulo + conversión de `assignPlan`

**Wave 7** _(blocked on Wave 6 completion)_

- [ ] 176-09-PLAN.md — Conversión de `changePlanAfterCurrent` y `changePlanNow` (boarding sí, AURA no) con diff cero del golden

**Wave 8** _(blocked on Wave 7 completion)_

- [ ] 176-10-PLAN.md — `getPricingPreview` con `commit:false` + merge neutro del `moduleOutput`, limpieza del tipo core y del constructor, y test de módulo apagado

**Wave 9** _(blocked on Wave 8 completion)_

- [ ] 176-11-PLAN.md — `enabledModules` en `/api/auth/me` (D-08, API-only), smoke post-deploy sobre ruta templo, doc 04 sincronizado y checkpoint humano
...
| 173. Adopción 2 — members + guarda de anclas      | 23/31          | In Progress |            |
| 174. Adopción 3a — subs+sched: código             | 0/?            | Not started |            |
| 174.1 Adopción 3b — batería ISO + switch          | 0/10           | Planned     |            |
| 175. Adopción 4a — analytics+resto: código        | 0/?            | Not started |            |
| 175.1 Adopción 4b — batería, switch y cierre      | 0/?            | Not started |            |
| 176. Módulos — flags, requireModule y registry    | 0/11           | Planned     |            |
...
### Phase 177: Paquete de clases — alta de corto plazo a medida por fórmula

**Goal:** Vender paquetes de clases de corto plazo (1–3 semanas × 1–6 clases/semana) desde el alta, con precio calculado por fórmula, sin introducir un motor de precios en runtime. Modelo B: matriz de planes reales generada por migración (categoría nueva `paquete`, `bookingMode='flexible'`, tope semanal duro), oculta del picker normal; la UI muestra un solo producto con 2 selectores (semanas, clases/semana) que resuelven al `planId`. Reutiliza budget derivado, cobertura y renovación nativos — no toca `assignPlan`.
**Requirements**: Ver `BRIEF-PAQUETE-CLASES.md` (raíz del repo) — decisiones cerradas, fórmula, grilla de 18 celdas y preguntas abiertas.
**Depends on:** Standalone — NO depende del tren v6.0 (166–175.1). Única coordinación: la numeración de la migración del enum `paquete` + los 18 INSERT debe tomar el número libre real al momento de tocar (el tren anda por 0198+).
**Plans:** 6/5 plans complete

Plans:

- [x] 177-01-PLAN.md — Base: enum `paquete` (Drizzle+API+admin) + migración de la matriz 36 filas (18 ARS + 18 EUR, grillas D-07/D-08) + helpers de categoría [wave 1]
- [x] 177-02-PLAN.md — Exclusión de paquete de descuentos Aura y referidos en todos los paths de cobro (D-13) [wave 2]
- [x] 177-03-PLAN.md — UI PoS profe: tab "Paquete de clases" con 2 selectores en CobrosPage + ocultar del picker normal [wave 2]
- [x] 177-04-PLAN.md — UI admin: modo paquete en AssignPlanDialog (alta + renovación D-11) + ocultar del tier grouping [wave 2]
- [x] 177-05-PLAN.md — Tests de integración: budget derivado, endDate, pricePaid por país, tope semanal duro en flexible (D-02), covered-until, exclusión descuentos (D-13), analytics cuenta (D-10) [wave 3]

### Phase 178: Bloque alternativo en técnica/combos + dos deuteros en pantalla del TV + baja de la rotación automática

**Goal:** En días técnica/combos, generar y editar un bloque alternativo al 2º bloque (`COMBOS_II_ALT`/`TECNICA_II_ALT`) que en el TV se muestra como toggle de vista compartiendo cronómetro; en días regulares, mostrar los dos deuteros juntos (2×2) y eliminar la rotación automática. Diseño y decisiones cerradas en `~/.claude/plans/atomic-painting-beaver.md`.
**Requirements**: TBD (derivar en plan-phase)
**Depends on:** Standalone (NO depende del tren v6.0 166–175.1 ni de la 177). Base: worktree `et-bloque-alt` / `feat/tv-bloque-alternativo` desde `origin/master`.
**Plans:** 7 plans (planificados 2026-08-19, plan-check PASSED; viven en el worktree `et-bloque-alt` / `feat/tv-bloque-alternativo`)

Plans:

- [ ] 178-01 — api: `COMBOS_II_ALT`/`TECNICA_II_ALT` en `BlockRole` + mapped types exhaustivos + conteo 4→5 (wave 1)
- [ ] 178-02 — Labels: 3 diccionarios espejo (api TV+badge, admin, app) (wave 1)
- [ ] 178-03 — Generadores combos/técnica emiten el 5º bloque alt (ruta role-inclusive → ejercicios distintos) + tests (wave 2, dep 01)
- [ ] 178-04 — Migración 0206 (drop rotación cols, add `show_alternative`) + schema `tv.ts` (wave 1)
- [ ] 178-05 — PDF admin: página del bloque alt (wave 2, dep 02)
- [ ] 178-06 — TV backend: deuteros 2×2 + toggle alt (displayRole) + baja de rotación + tests (wave 3, dep 01/02/03/04)
- [ ] 178-07 — TV frontend: grilla 2×2 + botón "Ver alternativo" + checkpoint humano visual (wave 4, dep 06)

### Phase 179: Referidos de partners/marcas: entidad partner con código/QR propio, beneficio configurable (descuento % o pase gratis), comisión fija one-time por membresía confirmada y liquidación manual en admin

**Goal:** Un comercio partner con código/QR propio trae socios: quien se registra con su código recibe el beneficio configurado (descuento % en la primera cuota o semana gratis de 3 clases), el partner acumula una comisión fija por cada membresía confirmada, y el admin gestiona partners, ve conversiones y liquida comisiones — todo tenancy-native y sin tocar el módulo de referidos socio-a-socio.
**Requirements**: D-01…D-20 (decisiones lockeadas en 179-CONTEXT.md; esta fase está fuera del milestone v6.0 y no reclama IDs de REQUIREMENTS.md)
...
- [x] 179-01-PLAN.md — Schema de las 3 tablas + migración 0209 + gates de tenancy

**Wave 2** _(blocked on Wave 1 completion)_

- [x] 179-02-PLAN.md — Servicio del módulo (CRUD tenancy-native) + resolver unificado de códigos

**Wave 3** _(blocked on Wave 2 completion)_

- [x] 179-03-PLAN.md — Rutas admin del CRUD con RBAC + manifiesto ISO-01

**Wave 4** _(blocked on Wave 3 completion)_

- [x] 179-04-PLAN.md — Atribución de partner en el alta + exclusividad de origen

**Wave 5** _(blocked on Wave 4 completion)_

- [x] 179-05-PLAN.md — Cualificación y comisión en las 4 charge-paths

**Wave 6** _(blocked on Wave 5 completion)_

- [x] 179-06-PLAN.md — Descuento one-shot en assignPlan + gana-el-mayor vs AURA

**Wave 7** _(blocked on Wave 6 completion)_

- [x] 179-07-PLAN.md — Descuento en cambio/renovación + paridad preview↔cobro

**Wave 8** _(blocked on Wave 7 completion)_

- [x] 179-08-PLAN.md — Semana gratis: elegibilidad, activación al reservar y precedencia sobre el trial

**Wave 9** _(blocked on Wave 8 completion)_

- [x] 179-09-PLAN.md — Void automático de comisiones + asignación retroactiva y revocación

**Wave 10** _(blocked on Wave 9 completion)_

- [x] 179-10-PLAN.md — Liquidación batch + reporte de conversiones y de beneficios sin conversión

**Wave 11** _(blocked on Wave 10 completion)_

- [x] 179-11-PLAN.md — Admin: página /partners con CRUD, ruta, nav y RBAC

**Wave 12** _(blocked on Wave 11 completion)_

- [x] 179-12-PLAN.md — URLs de tienda en system_settings + generador de los 2 QR con el código impreso

**Wave 13** _(blocked on Wave 12 completion)_

- [x] 179-13-PLAN.md — Admin: tabs de Conversiones y Liquidación

**Wave 14** _(blocked on Wave 13 completion)_

- [x] 179-14-PLAN.md — Admin: partner en la ficha del alumno + descuento en el diálogo de plan

**Wave 15** _(blocked on Wave 14 completion)_

- [x] 179-15-PLAN.md — App: campo unificado de código en el registro

**Wave 16** _(blocked on Wave 15 completion)_

- [x] 179-16-PLAN.md — App: estado de semana de partner en ReservasPage + bump de versión

**Wave 17** _(blocked on Wave 16 completion)_

- [ ] 179-17-PLAN.md — Verificación integral + checkpoints humanos (URLs, UAT AR/ES, shipping)

### Phase 180: Campañas etapa 2: magic links por sede, selector de audiencias y ReservasPage informativa para nuevos

**Goal:** Etapa 2 del sistema de campañas (fase 119): que el email lleve al freemium directo a reservar sin fricción de login y que el remitente elija a quién le habla. (1) **Magic links por sede** — token de un solo uso, expira 30d, loguea al destinatario (revisar D-21); CTA con UN botón primario personalizado a la sede del freemium (listEligible ya trae branch), tabla de sedes con links secundarios por fila, WhatsApp terciario. Motivación: con JWT 7d todos los freemium están deslogueados → el login mata el funnel click→reservó (el tracking ya mide ese salto). (2) **Selector de audiencias** — bajas, prueba-no-convertida, alerta/ausente (fase 136), referidos. (3) **ReservasPage más informativa para nuevos** (scope aprobado por Nacho 2026-08-20): dirección + "Cómo llegar" (Maps) en selector de sede (`branches.address` ya existe); card colapsable "¿Primera vez? Qué esperar"; confirmación post-reserva enriquecida (dirección+mapa, checklist, "Agregar al calendario" anti no-show); descripciones por actividad (columna `description` nueva + edición en admin + bottom sheet en el slot). Decisión: TODAS las clases aptas para todos los niveles → mensaje global, SIN chips de nivel.

> ⚠ **El brief de arriba quedó revisado por `180-CONTEXT.md` (2026-08-22).** Cambios que mandan: el token de login es **multi-uso y de 7 días** (D-02, no one-use 30d — el prefetch de escáneres de correo quema los one-use); el email lleva **UN solo CTA** y la tabla de sedes queda informativa, sin links por fila (D-06) porque la elección de sede se mudó a un popup dentro de la app (D-07); `activities.description` **ya existe** (no es columna nueva) y las etiquetas derivadas Combos/Técnica tienen descripción propia (D-23); entra además el recordatorio 24h pre-sesión con fallback por email (D-20/D-24).

**Requirements**: ML-01..ML-04, AUD-01..AUD-03, RES-01..RES-05, NOT-01, GOV-01 (IDs locales derivados en `180-RESEARCH.md`; no hay IDs en REQUIREMENTS.md para esta fase)
**Depends on:** Tren v6.0 en master (toca member app → bump minor + builds de tiendas, un solo pasaje). Standalone respecto de la 179.
**Plans:** 16 plans
...
- [ ] 180-01-PLAN.md — Token de campaña con propósito: login 7d + tracking 30d (+ worktree y base verificada)

**Wave 2** _(blocked on Wave 1 completion)_

- [ ] 180-02-PLAN.md — `GET /members/scheduling/branches` con dirección y link de Maps
- [ ] 180-03-PLAN.md — Recordatorio 24h pre-sesión: push + fallback por email
- [ ] 180-04-PLAN.md — Resolver de audiencias (5 segmentos) + columna `campaigns.segment` + migración

**Wave 3** _(blocked on Wave 2 completion)_

- [ ] 180-05-PLAN.md — App: popup de sedes y selector con dirección
- [ ] 180-06-PLAN.md — `POST /api/campaigns/exchange`: canje token→sesión + destinos por segmento + gates de tenancy

**Wave 4** _(blocked on Wave 3 completion)_

- [ ] 180-07-PLAN.md — App: card "¿Primera vez?", confirmación enriquecida y botón de calendario
- [ ] 180-08-PLAN.md — Envío por segmento persistido + `eligible-count` con COUNT(\*)
- [ ] 180-10-PLAN.md — Descripción de actividad coherente con la etiqueta derivada (Combos/Técnica)

**Wave 5** _(blocked on Wave 4 completion)_

- [ ] 180-09-PLAN.md — Email: CTA único de magic link + tabla de sedes informativa
- [ ] 180-11-PLAN.md — App: canje del magic link (composable, `/r/trial`, guard y `/volver`)
- [ ] 180-13-PLAN.md — App: bottom sheet de descripción de actividad
- [ ] 180-14-PLAN.md — Admin: selector de segmento y preview de audiencia
- [ ] 180-15-PLAN.md — Admin: edición de las descripciones de Combos y Técnica

**Wave 6** _(blocked on Wave 5 completion)_

- [ ] 180-12-PLAN.md — App: canje en el listener de deep links nativos + tests

**Wave 7** _(blocked on Wave 6 completion)_

- [ ] 180-16-PLAN.md — Cierre: gate de las 3 apps, verificación de nginx, UAT y bump minor
