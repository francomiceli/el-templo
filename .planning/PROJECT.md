...
## Previous Milestone: v5.8 Sesiones de Prueba — automatización y self-service

**COMPLETO 2026-07-16** (fases 163-165, 13/13 planes, verificación 163: 9/9, 164: 12/12, 165: 10/10). Migraciones 0182-0183. Pusheado a staging (`e2da7a7e`, con merge de deudas 0181); pendiente: dry-run 0183 contra prod (≈112 flips), UAT visual (HUMAN-UAT.md de las 3 fases), tren a master + bump 1.7.0.

**Goal:** Que el estado del lead de sesión de prueba se actualice solo (Ganado ya es automático; falta vencer a Perdido por ventana calculada del histórico y resetear al reprogramar), que la reprogramación sea una acción de primera clase con historial visible, y que el flujo self-service freemium existente (Phase 119, en prod sin UAT) quede validado y completado con teléfono obligatorio y mejoras de UX para gestión.

**Target features:**

- **Máquina de estados automática (foundational):** cron diario nuevo en `src/jobs/` que vence En seguimiento → Perdido cuando pasó la última sesión de prueba + X días sin compra; X = percentil 90 del histórico de días sesión→primera suscripción de los Ganados, sembrado y configurable en `system_settings`; reset Perdido → En seguimiento al agendar nueva SP (admin o self-service); columna `lead_status_source` (auto/manual) para auditar overrides; backfill de los ~112 En seguimiento vencidos por migración con tabla backup (precedente 0170) + dry-run validado contra los conteos del brief (211 Perdido / 136 En seguimiento / 105 Ganado sobre 452).
- **Reprogramación y reporte:** acción "Reprogramar" en el admin (cancela turno viejo + crea el nuevo en una transacción); contador de reprogramaciones derivado de bookings de prueba canceladas (sin schema nuevo, retroactivo gratis); indicador/filtro de estado manual vs automático en el reporte de Sesiones de Prueba.
- **Self-service y UX de gestión:** UAT + fixes del flujo freemium→prueba existente (registro → elegibilidad → reserva → reporte admin); teléfono obligatorio en toda reserva de SP (alta admin + diálogo self-service si el perfil no lo tiene); mejoras de experiencia para gestión al programar SP y convertir en alumno (levantar detalles con Nacho en discuss-phase).

**Decisiones clave (repaso del brief con Franco, 2026-07-15):**

- **El matching lead↔compra YA existe** — leads = `users` con status `prueba`, SP = `bookings` con `is_trial=1`, y el hook `recomputeUserStatus` ya marca Ganado + Plan comprado + `convertedAt` en la compra (los 4 flujos de cobro). El punto 4 del brief (identificador común) está resuelto por diseño; no se construye matching.
- **Ganado SIN ventana:** cualquier compra marca Ganado, incluso desde Perdido (recuperado). X solo gobierna el vencimiento a Perdido.
- **Guardrails ya existentes:** invariante Ganado⇔Plan validada en API+UI; "Plan comprado" ya es FK a planes; `lead_notes` ya es texto libre sin lógica. Solo falta el audit auto/manual.
- **"Quitar turno" ya hace soft-cancel** (`status='cancelado'`) — el historial de reprogramaciones existe en datos; solo se expone.

**Fuente:** `.docs/sp-auto/brief-fran-automatizacion-sesiones-prueba.md` (brief de Nacho, 2026-07-15) + 3 mapeos de codebase de esta sesión (sesiones de prueba, freemium/reservas, compras).

**Numeración:** fases **163+** (159-160 reservadas para v5.6 combos+técnica; 161-162 usadas por v5.7). Migraciones: verificar en plan-phase — última aplicada 0180, v5.6 reserva desde 0181.

## Previous Milestone: v5.7 Actividades con Aura

**EN PROD 2026-07-15** (tren `29816bf9`, migs 0179-0180, app 1.6.0). Pendientes: SUMMARYs+verifier de fases 161-162 (deuda GSD), carga de actividades/slots por ABM, imputación de 4 anticipos, builds de tiendas.
...
Pendiente — próximo milestone se define con `/gsd:new-milestone` (v5.8 archivado en `.planning/milestones/v5.8-REQUIREMENTS.md`; candidato en cola: v5.6 combos+técnica, fases 159-160 sin ejecutar)
