 ...
 *
 * Sends every DELETE as a single multi-statement query (one MySQL round-
 * trip total instead of ~50). FK constraints are disabled for the duration
 * so order doesn't matter. multipleStatements is enabled in the test pool
 * via NODE_ENV=test in plugins/database.ts; production keeps it disabled.
 *
