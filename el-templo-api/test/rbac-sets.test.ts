...
describe("RBAC sets — core white-label + Templo overrides", () => {
  ...
  it("ANALYTICS_ADMIN_ROLES is Dueño core + admin_sede (2026-09-09, UAT) — gestion stays excluded", () => {
    // El admin_sede entra a Analíticas admin-only (KPIs/miembros/financiero) pero
...
/**
 * 2026-09-08 — rol `admin_sede` (migración 0225).
 *
 * La regla del rol es "donde está `gestion`, está `admin_sede`", con las
 ...
 */
describe("RBAC — rol admin_sede (sede-scoped)", () => {
  const ENTRA = {
    ALL_STAFF_ROLES,
    CAJA_ROLES,
    COACH_DEBTS_ROLES,
  ...
  it.each(Object.entries(ENTRA))(
    "admin_sede ∈ %s (hereda la superficie de gestion)",
    (_name, set) => {
      expect([...(set as readonly string[])]).toContain(ADMIN_SEDE_ROLE);
    },
  );

  it.each(Object.entries(NO_ENTRA))(
    "admin_sede ∉ %s (dueño / entrenamiento / jornada / TV)",
    (_name, set) => {
      expect([...(set as readonly string[])]).not.toContain(ADMIN_SEDE_ROLE);
    },
  );

  it("PROGRAMAS_LIST_ROLES deja de coincidir en valor con FINANCE_WRITE_ROLES", () => {
    // Eran byte-idénticos hasta 2026-09-08. `admin_sede` rompe la coincidencia a
