 ...
 *
 * Behavior under test (D-32 / D-33 / D-34):
 ...
 *     - Sets `lead_status = 'cerrado'`.
 *     - Prefills `lead_notes = '<plan.name>'` ONLY when notes are NULL/empty
 *       (admin-entered notes are NEVER overwritten).
 *
 *   The hook MUST NOT fire for non-trial users (no is_trial booking) and
 *   MUST NOT re-prefill `lead_notes` on a second assignment after the user
...
describe("Phase 114-03 — subscription conversion hook (lead_status + lead_notes)", () => {
  ...
  it("prefills lead_notes with plan name when notes are NULL (canonical happy path)", async () => {
    ...
    expect(after.leadStatus).toBe("cerrado");
    expect(after.leadNotes).toBe("Flex Basic");
  ...
  it("treats empty-string lead_notes as empty and fills with plan name", async () => {
    const userId = await createTrialLead({
      firstName: "Empty",
      lastName: "String",
      phone: "1140000003",
  ...
  it("is idempotent: re-assigning after conversion does NOT mutate lead_notes again", async () => {
    const userId = await createTrialLead({
      firstName: "Idem",
      lastName: "Potent",
      phone: "1140000005",
    ...
    // First conversion: fills lead_notes with "Flex Basic".
    const first = await assignPlanViaHttp(userId, basicPlanId);
    expect(first.statusCode).toBe(201);

    const afterFirst = await fetchLeadColumns(userId);
    expect(afterFirst.leadStatus).toBe("cerrado");
    expect(afterFirst.leadNotes).toBe("Flex Basic");
    ...
    for (const t of linkedTxIds) {
      await app.db
        ...
        .where(eq(financialTransactions.id, t.id));
    }
    ...
    // Now re-assign a DIFFERENT plan. Hook should NOT touch lead_notes
    ...
    // lead_status stays 'cerrado' (it was already cerrado from the first
    // conversion); lead_notes is NOT re-prefixed with "Flex Pro".
    expect(afterSecond.leadStatus).toBe("cerrado");
    expect(afterSecond.leadNotes).toBe("Flex Basic");
