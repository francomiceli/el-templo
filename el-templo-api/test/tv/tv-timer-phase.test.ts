...
const VECTORS: GoldenVector[] = [
  ...
  {
    name: "rom (sabado): cronometro libre que sube desde cero",
    params: { type: "rom", rounds: 4, restSeconds: 45 },
    spec: { kind: "countup" },
    samples: [
      // ROM ahora es countup: cuenta HACIA ADELANTE todo el bloque, sin reset
      // por ronda y sin fin (el profe avanza de bloque a mano).
      {
        elapsedMs: 0,
        phase: "run",
        displayMs: 0,
