...
async function subscriptionPartnerDiscount(subscriptionId: number): Promise<{
  ...
  const rows = await app.db.execute(
    sql`SELECT /* tenant-safe: evidencia del test — lectura por id de subscription creada por el propio test, se afirma sobre la fila cruda */ partner_discount_percent, partner_discount_amount, price_paid
