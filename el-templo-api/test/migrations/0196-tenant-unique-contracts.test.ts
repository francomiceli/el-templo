...
describe("Verificador de uniques e índices por tenant — gate fail-closed (CON-02, D-14)", () => {
  ...
  it("Test 10: el reporte cierra en 0 discrepancias y cubre las 99 tablas gym-owned", () => {
    ...
    // + trial_followups (cadencia de mensajes en Sesiones de Prueba,
    //   2026-09-26, tenancy-native en la 0241) = 99.
    expect(
      report.gymOwnedChecked,
      `El verificador solo pudo mirar ${report.gymOwnedChecked} de las 99 tablas gym-owned ` +
        ...
        `+ 2 de renovaciones de la 0237 + trial_followups de la 0241): faltan tablas en ` +
        `${report.database} y el resultado no es concluyente.`,
    ).toBe(99);
    expect(
