...
// El complemento del Reporte Deudas: saldos de plan (amount > 0) cuya
// subscription todavía NO arrancó (start_date > CURDATE()). Es plata que se
// espera cobrar cuando el plan empiece; hoy NO es deuda cobrable (por eso el
// hotfix la excluye de Deudas). Fuente: balances WHERE amount > 0 AND
// target_kind='subscription' INNER JOIN subscriptions con start_date futuro.
...
export interface ScheduledIncomeRow {
  ...
  amount: number;
  currency: string;
  /** Fecha en que el plan arranca (subscriptions.start_date). YYYY-MM-DD. */
  startDate: string;
  /** Mes de inicio, YYYY-MM (para agrupar los totales). */
  startMonth: string;
}

/** Total esperado por mes de inicio (para proyección de ingresos). */
...
export interface ScheduledIncomeFilters {
  branchId?: number;
  country?: "AR" | "ES";
  currency?: string;
  search?: string;
...
/**
 * monthlyTotals:
 *  - Non-owner (single currency por country scope): flat MonthlyIncomeTotal[].
 *  - Owner (multi-moneda posible): keyed por currency. NUNCA se suman monedas.
 * Ambos ordenados por mes ASC.
 */
