...
/**
 * Convierte un mapa {mes 'YYYY-MM' → monto} en la lista de MonthlyIncomeTotal
 * ordenada por mes ASC, con label en español ("Julio 2026"). Usado por el
 * reporte de Cobros esperados.
 */
function monthMapToSortedTotals(m: Map<string, number>): MonthlyIncomeTotal[] {
  return [...m.entries()]
    .sort((a, b) => a[0].localeCompare(b[0]))
    .map(([month, amount]) => {
      const [year, mo] = month.split("-");
      const label = `${MONTHS_ES_OB[Number(mo) - 1] ?? ""} ${year}`.trim();
...
export class ReportsService {
  ...
  /**
   * Complemento del Reporte Deudas: saldos de plan (amount > 0) cuya
   * subscription todavía NO arrancó (start_date > CURDATE()). Es plata que se
   * espera cobrar cuando el plan empiece — hoy NO es deuda cobrable, por eso
   * el hotfix la excluye de Deudas. Este reporte la surface aparte.
   *
   * Filtro = complemento exacto de `collectibleDebtCondition` restringido a
   * subscription rows positivas, de modo que Deudas ⊎ Cobros esperados cubre
   * todos los saldos de plan > 0 sin solapamiento.
   *
   * Scope/país igual que getOutstandingBalances. Orden por start_date ASC
   * (los próximos a arrancar primero). monthlyTotals agrupa el monto esperado
   * por mes de inicio (flat para non-owner, keyed por moneda para owner).
   */
  async getScheduledIncome(
    ...
    // INNER JOIN subscriptions: por definición todas las filas son de plan.
    const conds: SQL[] = [
      gt(schema.balances.amount, 0),
      eq(schema.balances.targetKind, "subscription"),
      sql`${schema.subscriptions.startDate} > CURDATE()`,
    ];
    if (filters.branchId !== undefined) {
      conds.push(eq(schema.subscriptions.branchId, filters.branchId));
    }
    ...
    const rawRows = await this.db
      .select({
        ...
        branchId: schema.subscriptions.branchId,
        branchName: schema.branches.name,
        subscriptionId: schema.balances.targetId,
        planName: schema.subscriptionPlans.name,
        amount: schema.balances.amount,
        currency: schema.balances.currency,
      ...
      .leftJoin(schema.users, eq(schema.users.id, schema.balances.memberId))
      .where(whereClause)
      .orderBy(sql`${schema.subscriptions.startDate} ASC`)
      .limit(limit)
      .offset(offset);

    const rows: ScheduledIncomeRow[] = rawRows.map((r) => {
      const startDate = String(r.startDate);
      return {
        memberId: r.memberId,
        memberName:
    ...
    // ── monthlyTotals: monto esperado por mes de inicio, sobre TODO el set
    // filtrado (no solo la página). Un query proyectando lo mínimo.
    const totalsRows = await this.db
      .select({
        currency: schema.balances.currency,
        amount: schema.balances.amount,
    ...
    let monthlyTotals:
      | MonthlyIncomeTotal[]
      | Record<string, MonthlyIncomeTotal[]>;
    if (scope.isOwner) {
      const byCurrency: Record<string, Map<string, number>> = {};
      for (const r of totalsRows) {
        const month = String(r.startDate).slice(0, 7);
        const cur = r.currency;
        (byCurrency[cur] ??= new Map()).set(
          month,
          (byCurrency[cur].get(month) ?? 0) + Number(r.amount),
        );
      }
      monthlyTotals = Object.fromEntries(
        Object.entries(byCurrency).map(([cur, m]) => [
          cur,
          monthMapToSortedTotals(m),
        ]),
      );
    } else {
      ...
      for (const r of totalsRows) {
        const month = String(r.startDate).slice(0, 7);
        m.set(month, (m.get(month) ?? 0) + Number(r.amount));
      }
      monthlyTotals = monthMapToSortedTotals(m);
    }

    return { rows, total, page, limit, monthlyTotals };
  }
